...
About *using* the `nightrail` gem from a Rails application; copy
...
| `bin/rails 'nightrail:sourcemaps[public,true]'` | Uploads Vite source maps for the configured deploy, then deletes acknowledged files. Run after building and before publishing assets. Omit `true` to retain files. See [Source maps](docs/source-maps.md). |
