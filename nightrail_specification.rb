...
require_relative "lib/nightrail/version"

module Nightrail
  module Packaging
    PUBLIC_NAME = "nightrail"
    ...
    def specification(name: PUBLIC_NAME)
      Gem::Specification.new do |spec|
        spec.name        = name
        spec.version     = Nightrail::VERSION
        spec.authors     = [ "Cole Robertson" ]
        spec.email       = [ "cole@rebulk.com" ]
        spec.homepage    = "https://nightrail.rebulk.com"
        spec.summary     = "First-class monitoring for Rails applications."
        spec.description = "Nightrail instruments a Rails app end to end (requests, jobs, queries, exceptions, cache, mail, broadcasts, outgoing HTTP, logs) and ships linked events to Nightrail Cloud."
        spec.license     = "MIT"
