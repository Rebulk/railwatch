...
Lantern is the same product shape for Rails: one package instruments the
...
how to read Lantern — this page maps the vocabulary and points out the
...
Puma and Solid Queue workers *are* long-lived, so Lantern skips that
...
| `outgoing-request` | `outgoing_request` | One `Net::HTTP` prepend covers Faraday's default adapter, HTTParty, RestClient, and `ruby-llm`; `Lantern::Faraday` is the Guzzle-middleware equivalent for other adapters. |
...
| deployment (`nightwatch:deploy`) | `Deploy` on the platform | Posted by `lantern:deploy` or the Kamal `post-deploy` hook, with up to 50 commits so the platform can diff what shipped. |
| request `stages` | parent `stages` | Same idea, Rails boundaries: `middleware_before`, `action`, `render`, `middleware_after`, `body`. Laravel's `bootstrap` has no equivalent in a warm process — Lantern reports boot time once per process as a `process` record instead. |
...
`.exceptions`, `.scheduled_tasks`), decided once per execution. Lantern
...
class ReportsController < ApplicationController
  ...
  lantern_never_sample only: :health
end
```

Both take the same options as `before_action`. Programmatically:
`Lantern.sample(rate)`, `Lantern.dont_sample`, `Lantern.sampling?`.

**What Lantern adds: tail sampling.** Head sampling throws away exactly
...
the end if it ran that long, raised, or called `Lantern.keep!`. Its
...
Lantern.keep!                   # ...plus this one, whatever the roll said
```
...
| Nightwatch | Lantern |
|---|---|
| `Nightwatch::user($cb)` | `Lantern.user { \|user\| ... }` (or `c.user { }`) |
| `sample($rate)` | `Lantern.sample(rate)` |
| `dontSample()` | `Lantern.dont_sample` |
| `sampling()` | `Lantern.sampling?` |
| `ignore($cb)` | `Lantern.ignore { }` |
| `pause()` / `resume()` / `paused()` | `Lantern.pause` / `Lantern.resume` / `Lantern.paused?` |
| `report($e, $handled)` | `Lantern.report(error, handled: true)` |
| `redactRequests` / `redactQueries` / `redactExceptions` / `redactCacheEvents` / `redactCommands` / `redactMail` / `redactOutgoingRequests` | `Lantern.redact_requests`, `redact_queries`, `redact_exceptions`, `redact_cache_events`, `redact_commands`, `redact_mail`, `redact_outgoing_requests`, plus `redact_logs` |
| `rejectQueries` / `rejectCacheEvents` / `rejectMail` / `rejectNotifications` / `rejectOutgoingRequests` / `rejectQueuedJobs` | `Lantern.reject_queries`, `reject_cache_events`, `reject_mail`, `reject_notifications`, `reject_outgoing_requests`, `reject_enqueued_jobs`, plus `reject_broadcasts` and `reject_logs` |
| `rejectCacheKeys([...])` | `Lantern.reject_cache_keys(%w[session: rack::attack*])` |
| `captureDefaultVendorCommands` / `CacheKeys` | `c.capture_default_vendor_commands` / `c.capture_default_vendor_cache_keys` |
| `guzzleMiddleware()` | `Lantern::Faraday` (`Faraday.new(url) { \|f\| f.use Lantern::Faraday }`); `Net::HTTP` is covered globally with no setup |
| `IngestingEvents` listener returning `false` | `Lantern.before_ingest { \|batch\| ... }` |

Beyond the facade: `Lantern.context(**attrs)` (Laravel Context's
counterpart, writing through to all three of Rails' own context stores),
`Lantern.span(name, **attrs) { }`, `Lantern.keep!`,
`Lantern.attach(name, data)`, `Lantern.fingerprint { }`,
`Lantern.instrument_outgoing(method, url) { }`,
`Lantern.on_unrecoverable { }`, and `Lantern.flush`.

## Config and commands

`config/nightwatch.php` becomes `config/initializers/lantern.rb`, and
every setting still has an env var — `NIGHTWATCH_*` becomes `LANTERN_*`.
...
| `nightwatch:status` | `bin/rails lantern:status` |
| `nightwatch:deploy {deploy} --ref --name --url` | `bin/rails lantern:deploy[ref,name,url]`, or the generated `.kamal/hooks/post-deploy` |
| — | `bin/rails lantern:doctor`, which checks the whole install and exits non-zero if the token or the ingest host is wrong |

## The three things worth knowing about Rails

**Tenancy is first class.** Nightwatch tells you to prefix user ids by
hand. Lantern reads `ActiveRecord::Base.current_tenant` /
