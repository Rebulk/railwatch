...
Start with `bin/rails lantern:doctor`. It checks every piece of the
...
| `engine mounted` | `mount Lantern::Engine, at: "/lantern"` is missing from `config/routes.rb`; the browser beacon has nowhere to post. |
...
| `kamal post-deploy hook` | `.kamal/hooks/post-deploy` is missing or doesn't mention Lantern. Only matters if you deploy with Kamal. |
| `browser client` | `app/frontend/lib/lantern.ts` isn't there. Only matters for Inertia visit timing. |
| `browser client imported` | The client exists but nothing calls `startLantern()` — no `startLantern` found in `app/frontend/entrypoints`. Visits won't report. |
| `profiler backend` | Neither `vernier` nor `stackprof` is installed, so `Lantern::Profiler.available?` is false and the profiling settings are inert. |
| `test matchers` | Neither `spec/rails_helper.rb` requires `lantern/rspec` nor `test/test_helper.rb` requires `lantern/minitest`. |
...
different angles — Lantern decided not to record.

**The token is missing or blank.** `Lantern.enabled?` is
`config.enabled && token.present?`. With no token the engine's
`lantern.subscribe` initializer returns early, so no subscribers and no
patches are installed at all — this is by design, so the gem is inert in
development. Fix: set `LANTERN_TOKEN`, restart, re-run `lantern:doctor`
...
`lantern:doctor`'s `ingest reachable` line catches this before you
deploy.

**`LANTERN_INGEST_URL` points somewhere else.** Records go where you sent
them. `lantern:status` prints the URL it is actually using; compare it
against the platform you're looking at. Self-hosting: see
[`self-hosting.md`](self-hosting.md).

**`config.enabled` is false.** `LANTERN_ENABLED=0` (or `false`/`no`/`off`)
turns everything off with a valid token present.

**Sample rates are at zero.** `c.sample = { requests: 0.0 }` — or the
per-route `lantern_never_sample` macro on a controller — means no
...
**You're looking at the test environment.** Requiring `lantern/rspec`
(or `lantern/minitest`) swaps the reporter's transport for an in-memory
...
Still nothing? Set `LANTERN_DEBUG=1` and restart. Internal diagnostics go
to stderr prefixed `[lantern]` (never to `Rails.logger`, so they can't
become `log` records about themselves). `Lantern.on_unrecoverable { |e|
...
Lantern doesn't dedupe: it records one `scheduled_task` per
...
**Symptom.** `have_lantern_outgoing_requests` never sees anything, and no
`outgoing_request` records appear from the test suite. Production is
fine.

**Cause.** WebMock replaces `::Net::HTTP` with a subclass whose
`#request` short-circuits before calling `super`, so Lantern's prepend on
...
`Lantern::Subscribers::ProcessInfo.role`, in this order: `worker` when
...
- `c.buffer_size` (default 5,000) caps the process-wide queue between
  the app and the reporter thread. Oldest-dropped-first, also counted.
...
`c.ignore`, or use `Lantern.keep!` on the specific paths you care about
...
**Cause.** No profiler backend is installed. Lantern doesn't vendor one:
`Lantern::Profiler.available?` is false unless `vernier` or `stackprof`
...
- **`LANTERN_TOKEN` isn't exported to the hook.** The first thing
  `.kamal/hooks/post-deploy` does is `[ -z "$LANTERN_TOKEN" ] && exit 0`.
...
- **`curl` or `ruby` isn't on the deployer, or `LANTERN_INGEST_URL`
  isn't set.** Then the hook falls back to
  `bin/kamal app exec --primary --reuse "bin/rails lantern:deploy[$KAMAL_VERSION]"`,
...
- **The hook isn't there.** The install generator only writes it when
  ...
  `bin/rails generate lantern:install` after adopting Kamal.
...
`Lantern::Context.current_tenant`, and a tenant only exists as a GROUP BY
over those rows. If nothing ever sets it, every row has a null tenant and
there is nothing to group.

**Fix.** Apps on `activerecord-tenanted` get it free — Lantern reads
...
```ruby
Lantern.context(tenant: org.slug)
```
