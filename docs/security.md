...
application. Lantern Cloud is a separate service and is outside this review.

## Transport

`Lantern::Transport::Http` sends gzip NDJSON with `Net::HTTP`. HTTPS explicitly
...
`::1`, or `LANTERN_ALLOW_HTTP=true` is explicitly set. Refusal does not raise
into application code: delivery returns a failed result, `lantern:doctor`
...
The defaults are defined in `Lantern::Configuration` and covered by
...
- `capture_exception_source` is true. Source lines surrounding in-application
  backtrace frames are sent to Lantern Cloud. Disable it if source context is
  outside the application's telemetry policy.

Log record messages are sent exactly as supplied to `Rails.logger`. Lantern
...
log secrets; `Lantern.redact_logs` can implement an application-specific scrub,
and `Lantern.reject_logs` or `LANTERN_IGNORE_LOGS=true` can omit log records.

## Browser beacon

`POST /lantern/beacon` is the gem's only inbound unauthenticated endpoint. It
