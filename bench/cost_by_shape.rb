...
#   RAILWATCH_REQUEST_SAMPLE_RATE=0 bundle exec ruby bench/cost_by_shape.rb   # head-sampled-out path
...
label = Railwatch.config.sample[:requests].zero? ? "sampled out" : "sampled in"
puts "Railwatch #{label}: per-request CPU µs on the request thread (off = gem unsubscribed), #{ROUNDS} interleaved rounds x #{BATCH}"
