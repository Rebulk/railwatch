...
# with Railwatch genuinely out of the way and then in, interleaved, and
...
failures = []
puts format("%-34s %9s %9s %9s %9s | %7s %7s %7s", "added by Railwatch (sampled in)", "off p50", "on p50", "Δ p50 µs", "Δ min µs", "off al", "on al", "Δ al")
...
puts "Rails.logger.debug? with Railwatch installed: #{Rails.logger.debug?} (must stay false, or every framework log line is formatted)"
failures << "Railwatch's log capture turned Rails.logger.debug? on" if Rails.logger.debug?
