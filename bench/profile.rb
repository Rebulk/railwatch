...
# Attribution: which Lantern frames the per-request and per-query cost
...
# request and prints only the Lantern lines, then stackprof (wall) over
...
def stackprof_lines(mode:, interval:, top:)
  path = "/tmp/lantern_bench_profile.dump"
...
{ "trivial request" => "/bench/trivial", "20 uncached queries" => "/bench/queries?n=20" }.each do |label, path|
  ...
  lantern = report.allocated_objects_by_location.select { |h| h[:data].start_with?(GEM) }
  puts "\n== #{label}: #{report.total_allocated / 20} objects/request, #{lantern.sum { |h| h[:count] } / 20} allocated directly in lib/lantern"
  lantern.first(14).each { |h| puts format("  %5d  %s", h[:count] / 20, h[:data].delete_prefix(GEM)) }
  ...
  puts lines.select { |l| l.include?("lantern/") || l.include?("Lantern") }.first(14).map { |l| "  " + l.rstrip }
end
