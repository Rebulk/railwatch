...
run_config() {
  local label="$1"; shift
  env RAILS_ENV=test RAILWATCH_TOKEN=bench RAILWATCH_INGEST_URL="$SINK" "$@" \
    bundle exec puma -p "$PORT" -t 4:4 -w 0 -q spec/dummy/config.ru > /tmp/railwatch_load_puma.log 2>&1 &
  local pid=$!
  ...
  echo "== $label"
  local rss0 cpu0 rep0; rss0=$(awk '/VmRSS/{print $2}' /proc/$pid/status); cpu0=$(proc_cpu $pid); rep0=$(thread_cpu $pid railwatch-report)
  for p in $PATHS; do
    ruby bench/load/loadgen.rb -u "http://127.0.0.1:$PORT$p" -c "$CONC" -d "$DUR"
  done
  sleep 3  # let the reporter flush its last interval
  local rss1 cpu1 rep1; rss1=$(awk '/VmRSS/{print $2}' /proc/$pid/status); cpu1=$(proc_cpu $pid); rep1=$(thread_cpu $pid railwatch-report)
...
for round in $(seq 1 "$ROUNDS"); do
  ...
  run_config "disabled (RAILWATCH_ENABLED=0)" RAILWATCH_ENABLED=0
  run_config "enabled, sampled in" RAILWATCH_ENABLED=1
  run_config "enabled, requests sampled 10%" RAILWATCH_ENABLED=1 RAILWATCH_REQUEST_SAMPLE_RATE=0.1
  run_config "enabled, sampled out" RAILWATCH_ENABLED=1 RAILWATCH_REQUEST_SAMPLE_RATE=0
done
