...
# Nightrail's SQL subscribers add in each execution state, then the pieces of
...
rows = []
sql_subs = NIGHTRAIL_SUBSCRIPTIONS.select { |p, _, _| p == "sql.active_record" }
sql_subs.each { |_, s, _| ActiveSupport::Notifications.unsubscribe(s) }

rows << [ "sql.active_record instrument, Rails' AR LogSubscriber only (Nightrail unsubscribed)", per_call(&notify) ]
empties = []
(1..3).each do |n|
...
rows << [ "  Nightrail log Capture detached (Rails.logger.debug? => #{Rails.logger.debug?})", per_call(&notify) ]
...
rows << [ "Nightrail sql subscribers, no execution (query outside a request/job): builds and drops", per_call(&notify) ]
...
adapter, db, = Nightrail::Subscribers::Queries.connection_info(conn)
group, = Nightrail::SqlNormalizer.group_and_normalized(sql, adapter: adapter, connection_name: db)
rows << [ "    SqlNormalizer cache hit", per_call { Nightrail::SqlNormalizer.group_and_normalized(sql, adapter: adapter, connection_name: db) } ]
rows << [ "    connection_info memo hit", per_call { Nightrail::Subscribers::Queries.connection_info(conn) } ]
rows << [ "    source_for cache hit", per_call { Nightrail::Subscribers::Queries.source_for(group, false) } ]
rows << [ "    Backtrace.caller_location (n+1 record, slow query, first sight of a group)", per_call(2_000) { Nightrail::Backtrace.caller_location(skip: 1) } ]
rows << [ "    exe.envelope memo hit", per_call { exe.envelope } ]
rows << [ "    Record.group_hash (MD5)", per_call { Nightrail::Record.group_hash("MemoryStore", "widgets/?") } ]
rows << [ "    Context.serialized (per log line, per parent, per exception)", per_call { Nightrail::Context.serialized } ]
rows << [ "    Execution.new (sampled in)", per_call(5_000) { Nightrail::Execution.new(source: :request, sampled: true) } ]
