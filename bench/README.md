...
provides `nightrail_off!` / `nightrail_on!`: the gem's notification subscribers
...
| `no_db_writes.rb` | **Gate.** Did any INSERT/UPDATE/DELETE come from a frame inside `lib/nightrail`? |
| `cost_by_shape.rb` | The gate's measurement over eight shapes, ungated, with `ROUNDS` and `BENCH_OUT` for before/after diffs. `NIGHTRAIL_REQUEST_SAMPLE_RATE=0` for the head-sampled-out path. |
...
| `profile.rb` | Attribution: `memory_profiler` and `stackprof` over the trivial and 20-query requests, Nightrail frames only, then wall profiles of the middleware, a cache fetch, and a log line. |
