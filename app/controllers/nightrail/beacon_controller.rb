...
module Nightrail
  ...
  # client (app/frontend/lib/nightrail.ts). Mounted at /nightrail/beacon.
  class BeaconController < ActionController::API
    ...
    before_action :limit_payload, :throttle, if: -> { Nightrail.config.beacon_enabled }

    def create
      return head :no_content unless Nightrail.config.beacon_enabled
      ...
      Nightrail.debug { "discarded invalid beacon payload: #{error.class}: #{error.message}" }
      head :no_content
    end
    ...
    def throttle
      limit = Nightrail.config.beacon_rate_limit.to_i
      return unless limit.positive?

      count = cache_store.increment("nightrail:beacon:#{request.remote_ip}", 1, expires_in: RATE_LIMIT_WINDOW)
    ...
    # (startNightrail({ tenant })) fills that gap, and never overrides a
    ...
    # Whatever the app handed Nightrail's reportError -- a React error
