...
class Issue < RailwatchRecord
  ...
  def merge_into!(target)
    ...
    transaction do
      ...
      activities.create!(kind: "merge", user: ::Current.user, data: { target_id: target.id, target_key: target.key })
      target.activities.create!(kind: "absorbed", user: ::Current.user, data: { source_id: id, source_key: key, occurrences: occurrences })
    end
  end
  ...
  def unmerge!
    transaction do
      ...
      activities.create!(kind: "unmerge", user: ::Current.user, data: { target_id: target.id, target_key: target.key })
    end
  end
  ...
  def split!(by: "message")
    ...
    transaction do
      ...
      activities.create!(kind: "split", user: ::Current.user, data: { by: by, occurrences: moved, into: created.map(&:key) })
      created
    end
  end
  ...
  def fire_alerts!(event, extra = {})
    ...
    application.alert_rules.where(event: event).find_each do |rule|
      payload = { issue_key: key, title: title, environment: environment.name, actor: ::Current.user&.name }.merge(extra)
  ...
  def split_off(message, rows)
    ...
    issue.activities.create!(kind: "split", user: ::Current.user, data: { from_id: id, from_key: key, occurrences: rows.size })
    issue
  end
