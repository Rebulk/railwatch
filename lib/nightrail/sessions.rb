...
module Nightrail
  ...
  #
  # Same thread shape as Nightrail::Health -- one thread per process, parked on
  ...
  module Sessions
    ...
    MAX_KEYS = 10_000
    # The cookie the browser client sets (nightrail.ts), so every request from
    # a tab carries the id its beacons already use.
    COOKIE = /(?:\A|;\s*)nightrail_session=([^;]+)/
    KEY_LIMIT = 64
    ...
    def flush
      now = Clock.now
      timeout = Nightrail.config.session_timeout
    ...
    def key_for(exe, env)
      id = env["HTTP_X_NIGHTRAIL_SESSION"] || COOKIE.match(env["HTTP_COOKIE"])&.[](1)
    ...
    def emit(key, entry, ended)
      Nightrail.record(:session, group: Record.group_hash(key),
      ...
      Nightrail.debug { "session flush failed: #{e.class}: #{e.message}" }
      nil
    end
    ...
    # --- background thread (mirrors Nightrail::Health) ------------------------

    def start!
      return unless Nightrail.enabled? && Nightrail.config.track_sessions
      ...
      @mutex.synchronize do
        ...
        @thread.name = "nightrail-sessions"
    ...
    def run
      until @stopping
        @mutex.synchronize { @wakeup.wait(@mutex, Nightrail.config.session_flush_interval) unless @stopping }
