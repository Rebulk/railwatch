...
# Test helpers for apps using Nightrail. Add to spec/rails_helper.rb:
#   require "nightrail/spec_helper"
#   config.include Nightrail::SpecHelper
#
# `require "nightrail/rspec"` does both of those and adds the block matchers;
# `require "nightrail/minitest"` is the Minitest equivalent.
module Nightrail
  module SpecHelper
    ...
    def nightrail_records(type = nil)
      Nightrail.flush
      all = nightrail_transport.batches.flatten
      type ? all.select { |r| r[:t] == type.to_s } : all
    end

    def nightrail_transport
      @nightrail_transport ||= begin
    ...
    # matcher in nightrail/rspec and every assertion in nightrail/minitest.
    #
    ...
    def nightrail_capture
      unless Nightrail.enabled?
        raise Disabled, "Nightrail is disabled (config.enabled is false or config.token is blank), " \
                        "so this block would always look empty. Set NIGHTRAIL_TOKEN in your test environment."
      end

      Nightrail.flush
      batch_offset = nightrail_transport.batches.size
      buffered = with_nightrail_execution { yield }
      Nightrail.flush
      nightrail_transport.batches[batch_offset..].flatten + buffered
    end
    ...
    def with_nightrail_execution
      ...
      if exe
        ...
        Nightrail.start_execution(source: :command, sample_kind: :requests).sampled = true
        begin
          yield
        ensure
    ...
    # --- shared by nightrail/rspec and nightrail/minitest -------------------------
