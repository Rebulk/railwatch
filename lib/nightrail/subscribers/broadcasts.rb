...
module Nightrail
  module Subscribers
    ...
    module Broadcasts
      ...
      EXECUTION_KEY = :__nightrail_channel_execution
      ...
      class ActionExecution
        def start(_name, _id, payload)
          ...
          exe = Nightrail.start_execution(source: :channel_action, sample_kind: :channels,
          ...
          Nightrail.debug { "perform_action.action_cable start subscriber raised #{e.class}: #{e.message}" }
        ...
        def finish(_name, _id, payload)
          ...
          Nightrail.record(:broadcast, group: Record.group_hash(channel, action),
                         timestamp: exe.started_at, kind: "perform_action", channel: channel,
                         action: action, duration: exe.duration, failed: error ? true : false)
          Nightrail.finish_execution(:channel_action, group: Record.group_hash(channel, action),
                                   channel: channel, action: action,
                                   status: error ? "failed" : "processed", failed: error ? true : false)
        rescue StandardError => e
          Nightrail.debug { "perform_action.action_cable finish subscriber raised #{e.class}: #{e.message}" }
          Nightrail.notify_unrecoverable(e)
        ensure
          ...
          Nightrail::Current.execution = exe.parent_execution if exe && Nightrail.execution.equal?(exe)
        end
      end
      ...
      def install!(_app)
        ...
        subscribe("transmit.action_cable") do |event|
          ...
          next unless recording?
          p = event.payload
          Nightrail.record(:broadcast, group: Record.group_hash(p[:channel_class].to_s),
