  ...
  module Subscribers
    ...
    module Exceptions
      ...
      def install!(_app)
        ...
        Locals.install! if Nightrail.config.capture_exception_locals
        ...
        subscribe("rescue_from_callback.action_controller") do |event|
          next unless Nightrail.config.capture_rescued_exceptions
      ...
      # Opt-in (NIGHTRAIL_CAPTURE_EXCEPTION_LOCALS): a TracePoint on :raise
      ...
      module Locals
        ...
        def install!
          ...
          @trace = TracePoint.new(:raise) do |tp|
            ...
            next if error.instance_variable_defined?(:@__nightrail_locals)
            error.instance_variable_set(:@__nightrail_locals, snapshot(tp.binding))
        ...
        def for(error)
          error.instance_variable_get(:@__nightrail_locals)
        rescue StandardError
          nil
        end
      end
      ...
      def capture(error, handled:, severity:, context: {}, source: nil, fingerprint: nil)
        ...
        exe.session_crashed = true if exe && !handled && Nightrail.config.track_sessions
        ...
        cause = error.cause
        frames = Backtrace.frames(error, with_source: Nightrail.config.capture_exception_source)
        ...
        # (Nightrail.attach(exception:)) lands on the issue this call chose.
        ...
        rec = {
          ...
          locals: Nightrail.config.capture_exception_locals ? Locals.for(error) : nil,
          fingerprint: parts,
        ...
        if handled
          Nightrail.record(:exception, group: group, **rec)
        else
          ...
          # children (Nightrail.tail_keep?).
          exe.exception_reported = true if exe
          Nightrail.record_now(:exception, group: group, **rec)
        end
      end

      # The group hash `capture` would assign this error. Public so
      # Nightrail.attach can file an attachment against the same issue without
      # having to re-derive the bucketing rule (source snippets are skipped:
      # they cost I/O and don't take part in the hash).
      def group_for(error)
        parts = error.instance_variable_get(:@__nightrail_fingerprint) ||
      ...
      # an explicit `Nightrail.report(error, fingerprint: [...])` ("report"),
      # the error object's own #nightrail_fingerprint ("error"), the
      # `Nightrail.fingerprint { }` resolver ("resolver"), or Nightrail's own
      ...
      def fingerprint_for(error, top, override: nil)
        ...
        custom, source =
          ...
          elsif error.respond_to?(:nightrail_fingerprint) then [ error.nightrail_fingerprint, "error" ]
          elsif (resolver = Nightrail.config.fingerprint_resolver) then [ resolver.call(error, default), "resolver" ]
          end
        parts = custom && expand_fingerprint(custom, default)
        parts ? [ parts, source ] : [ cap_fingerprint(default), "default" ]
      rescue StandardError => e
        Nightrail.debug { "fingerprint for #{error.class} raised #{e.class}: #{e.message}; using the default" }
      ...
      # A custom fingerprint: `:default` splices in the parts Nightrail would
      ...
      def remember_fingerprint(error, parts)
        error.instance_variable_set(:@__nightrail_fingerprint, parts)
      rescue StandardError
        nil
      end
      ...
      def ignored?(error)
        ignored = Nightrail.config.ignored_exceptions
