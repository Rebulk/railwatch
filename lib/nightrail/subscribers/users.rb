...
module Nightrail
  module Subscribers
    # Resolves the current user. Default order: the app's Nightrail.user block,
    ...
    module Users
      ...
      def resolve_beacon_id(request)
        if (resolver = Nightrail.config.beacon_user_resolver)
      ...
      def describe(user)
        details = if (resolver = Nightrail.config.user_resolver)
          resolver.call(user)
        else
          {
      ...
      def remember(details)
        ...
        record = Nightrail.record(:user, id: key, name: details[:name], email: details[:email],
      ...
      # Called from Nightrail.finish_execution, for a tree that is being handed
