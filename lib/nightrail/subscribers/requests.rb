    ...
    module Requests
      ...
      def install!(_app)
        subscribe("start_processing.action_controller") do |event|
          ...
          env = p[:request]&.env
          next unless env
          env["nightrail.route"] = { pattern: route_pattern(p[:request]), controller: p[:controller].to_s.delete_suffix("Controller").underscore, action: p[:action], verb: p[:method] }
          ...
          env["nightrail.files"] = env["CONTENT_TYPE"]&.start_with?("multipart/form-data") ? uploaded_files(p[:request].params) : EMPTY_FILES
        end
        ...
        subscribe("process_action.action_controller") do |event|
          ...
          env["nightrail.view_runtime"] = p[:view_runtime]&.round(2)
          env["nightrail.db_runtime"] = p[:db_runtime]&.round(2)
          exe.enter_stage(:middleware_after)
          resp = p[:response]
          if resp && env["HTTP_X_INERTIA"] == "true"
            env["nightrail.inertia_props_bytes"] = resp.body.bytesize rescue nil
          end
        end

        subscribe("redirect_to.action_controller") do |event|
          env = event.payload[:request]&.env or next
          env["nightrail.redirect_to"] = Record.url_without_sensitive_components(event.payload[:location], limit: 512)
        end

        subscribe("halted_callback.action_controller") do |event|
          exe = execution or next
          ActiveSupport::ExecutionContext.to_h[:controller]&.request&.env&.[]=("nightrail.halted_callback", event.payload[:filter].to_s)
        end

        subscribe("unpermitted_parameters.action_controller") do |event|
          env = event.payload.dig(:context, :request)&.env or next
          env["nightrail.unpermitted_parameters"] = Array(event.payload[:keys]).map(&:to_s)
        end

        subscribe("rate_limit.action_controller") do |event|
          env = event.payload[:request]&.env or next
          env["nightrail.rate_limited"] = { name: event.payload[:name].to_s, count: event.payload[:count], to: event.payload[:to] }
        end
