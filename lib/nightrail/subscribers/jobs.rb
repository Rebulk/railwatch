    ...
    module Jobs
      ...
      def install!(_app)
        ...
        subscribe("perform_start.active_job") do |event|
          ...
          exe = Nightrail.start_execution(
            source: key ? :scheduled_task : :job,
            sample_kind: key ? :scheduled_tasks : :jobs,
            trace_id: job.respond_to?(:nightrail_trace_id) ? job.nightrail_trace_id : nil,
            parent_id: job.respond_to?(:nightrail_parent_id) ? job.nightrail_parent_id : nil,
          ...
          propagated_user = job.nightrail_user if job.respond_to?(:nightrail_user)
          propagated_tenant = job.nightrail_tenant if job.respond_to?(:nightrail_tenant)
          ...
          job.instance_variable_set(:@__nightrail_execution, exe)
          job.instance_variable_set(:@__nightrail_recurring_key, key)
        end

        subscribe("perform.active_job") do |event|
          job = event.payload[:job]
          exe = job.instance_variable_get(:@__nightrail_execution) or next
          Nightrail::Current.execution = exe
          exe.finish_stages
          p = event.payload
          released = job.instance_variable_get(:@__nightrail_released)
          ...
          key = job.instance_variable_get(:@__nightrail_recurring_key)
          fields = {
            job_id: job.job_id,
          ...
          if key
            Nightrail.finish_execution(:scheduled_task, group: Record.group_hash(key), task_key: key,
                                     schedule: schedule_for(key), drift: exe.drift, **fields)
          else
            Nightrail.finish_execution(:job_attempt, group: Record.group_hash(job.class.name), **fields)
          end
        end
        ...
        subscribe("enqueue_retry.active_job") do |event|
          ...
          p[:job]&.instance_variable_set(:@__nightrail_released, true)
          next unless recording?
          if Nightrail.config.capture_job_retry_errors && p[:error]
          ...
          Nightrail.record(:log, level: "warn", message: "Retrying #{p[:job].class.name} in #{p[:wait]}s: #{p[:error]&.class}",
        ...
        subscribe("fail_many_claimed.solid_queue") do |event|
          ...
          Array(p[:job_ids]).each do |job_id|
            Nightrail::Current.with(Nightrail::Execution.new(source: :job, sampled: true)) do
              Nightrail.record_now(:job_attempt, group: Record.group_hash("SolidQueue::Pruned"),
        ...
        subscribe("enqueue_recurring_task.solid_queue") do |event|
          p = event.payload
          next if p[:skipped]
          Nightrail.record(:log, level: p[:enqueue_error] ? "error" : "info",
      ...
      def captured_arguments(job)
        return {} unless Nightrail.config.capture_job_arguments
      ...
      def redact_arguments(arguments)
        Array(arguments).map do |argument|
          case argument
          when Hash then Nightrail.redactor.params(argument)
      ...
      def recurring_tasks
        ...
        if @recurring_tasks.nil? || now - @recurring_read_at > RECURRING_TTL
          @recurring_tasks = Nightrail.ignore { load_recurring_tasks }
