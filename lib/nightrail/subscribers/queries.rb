...
module Nightrail
  module Subscribers
    ...
    module Queries
      ...
      # each database gets its native plan format -- with Nightrail paused, and
      ...
      def explain_for(payload, duration_ms, group)
        return nil unless duration_ms >= Nightrail.config.explain_threshold_ms
        conn = payload[:connection] or return nil
        return nil unless SELECT.match?(payload[:sql])
        return nil unless explain_due?(group)

        plan = Nightrail.ignore { conn.explain(payload[:sql], payload[:binds] || []) }
      ...
      def install!(_app)
        subscribe("sql.active_record") do |event|
          ...
          # kwargs-packing into Nightrail.record and merging through
          # Record.build -- this is the hottest record type in the gem.
          Nightrail.push(:query, {
            v: QUERY_VERSION,
            t: "query",
          ...
          if exe && n == cfg.n_plus_one_threshold
            Nightrail.record(:n_plus_one, group: group, sql: normalized[0, 2048],
        ...
        subscribe("transaction.active_record") do |event|
          ...
          Nightrail.record(:transaction, group: Record.group_hash(connection_name, outcome),
        ...
        subscribe("strict_loading_violation.active_record") do |event|
          ...
          next unless recording?
          p = event.payload
          Nightrail.record(:log, level: "warn", message: "Lazy load of #{p[:owner].class.name}##{p[:reflection]&.name}",
