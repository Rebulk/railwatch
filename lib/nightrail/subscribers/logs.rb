  ...
  module Subscribers
    ...
    module Logs
      ...
      class Capture < ::Logger
        ...
        # Nightrail turn all of that formatting on for lines nobody stored.
        ...
        def add(severity, message = nil, progname = nil)
          return true unless Nightrail.enabled?
      ...
      class EventSubscriber
        def emit(event)
          ...
          return if !Nightrail.config.capture_framework_events && FRAMEWORK_EVENT_PREFIXES.any? { |p| name.start_with?(p) }
          Nightrail.record(:log,
      ...
      def min_severity
        level = Nightrail.config.log_level
      ...
      def write(level, message)
        ...
        return if text.start_with?("[nightrail]") || framework_noise?(text)
        exe.count(:logs)
        return unless recording?
        cfg = Nightrail.config
        # One hash literal instead of kwargs-packing into Nightrail.record --
