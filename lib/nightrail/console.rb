...
module Nightrail
  ...
  # production types typos, and a typo is not an issue. Nightrail subscribes to
  ...
  # `config.capture_console = true` (or NIGHTRAIL_CAPTURE_CONSOLE=1) for the
  # rare "trace what I'm about to do in here" session.
  #
  # A `rails runner` script is NOT this: it is a deployed execution and keeps
  # reporting -- see Nightrail::Patches::RunnerCommand for where that line is.
  module Console
    module_function
    ...
    def quiet?
      detected? && !Nightrail.config.capture_console
    end
    ...
    # reporter/health/sessions threads are already gated on Nightrail.enabled?.
    # Returns whether this call is what silenced the process.
    def silence!
      return false unless quiet? && Nightrail.config.enabled

      Nightrail.debug { "console detected -- capturing nothing in this process (set config.capture_console = true, or NIGHTRAIL_CAPTURE_CONSOLE=1, to capture a console session)" }
