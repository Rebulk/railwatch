...
module Nightrail
  module Middleware
    ...
    class Request
      ...
      def call(env)
        return @app.call(env) unless Nightrail.enabled?
        ...
        env["nightrail.execution"] = exe
        status = headers = body = nil
        begin
          status, headers, body = @app.call(env)
        rescue Exception => e # rubocop:disable Lint/RescueException
          Subscribers::Exceptions.capture(e, handled: false, severity: :error, source: "nightrail.middleware")
          raise
        ensure
      ...
      def ignored_request?(env)
        ...
        Nightrail.config.ignored_request_paths.any? do |pattern|
          pattern.is_a?(Regexp) ? pattern.match?(path) : pattern.to_s == path
        end || self_ingest_request?(env, path)
      end

      # Nightrail Cloud monitors itself. Its reporter therefore POSTs back into
      ...
      def self_ingest_request?(env, path)
        ...
        return false unless env["HTTP_AUTHORIZATION"] == "Bearer #{Nightrail.config.token}"

        endpoint = URI.join(Nightrail.config.ingest_url, "/ingest")
      ...
      def finish(env, exe, status, headers)
        ...
        Nightrail.finish_execution(:request) { parent_fields(env, exe, status, headers) }
        # A request with no user and no session cookie has no session, and
        # Sessions.touch returns without writing anything.
        Sessions.touch(exe, env, status) if Nightrail.config.track_sessions
      rescue StandardError => e
        Nightrail.debug { "request finish failed: #{e.class}: #{e.message}" }
        Nightrail.finish_execution
      end

      def parent_fields(env, exe, status, headers)
        req = ActionDispatch::Request.new(env)
        route = env["nightrail.route"] || {}
        ...
        payload = Nightrail.config.capture_request_payload && exe.counters[:exceptions].positive? ? Nightrail.redactor.params(req.filtered_parameters.except("controller", "action")) : nil

        {
          ...
          view_runtime: env["nightrail.view_runtime"],
          db_runtime: env["nightrail.db_runtime"],
          redirect_to: env["nightrail.redirect_to"],
          halted_callback: env["nightrail.halted_callback"],
          unpermitted_parameters: env["nightrail.unpermitted_parameters"],
          rate_limited: env["nightrail.rate_limited"],
          inertia: inertia,
          headers: request_headers(env),
          payload: payload,
          queue_time: queue_time(env, exe),
          user_agent: req.user_agent.to_s[0, 256],
          files: env["nightrail.files"] || uploaded_files_fallback(req, env)
        }
      end
      ...
      # here would make Nightrail the only component that ever reads that body,
      ...
      def inertia_fields(env, headers)
        ...
        return nil unless env["HTTP_X_INERTIA"] == "true" || (headers && (headers["X-Inertia"] || headers["x-inertia"])) || env["nightrail.inertia_component"]

        {
          component: env["nightrail.inertia_component"],
          ...
          props_bytes: env["nightrail.inertia_props_bytes"],
          ssr_ms: env["nightrail.inertia_ssr_ms"]
        }.compact
      end
