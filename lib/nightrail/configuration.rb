...
module Nightrail
  # All settings, each overridable by a NIGHTRAIL_* env var. Mirrors the shape of
  ...
  class Configuration
    ...
    # Nightrail's own browser transport in another request execution (the
    # beacon). Apps can replace this list through NIGHTRAIL_IGNORED_REQUEST_PATHS
    # or append exact paths/regexps in their initializer.
    DEFAULT_IGNORED_REQUEST_PATHS = %w[/up /nightrail/beacon].freeze
    ...
    # to Nightrail sees the same signal-to-noise out of the box.
    ...
    def initialize
      @enabled = env_bool("NIGHTRAIL_ENABLED", true)
      @token = ENV["NIGHTRAIL_TOKEN"]
      @ingest_url = ENV.fetch("NIGHTRAIL_INGEST_URL", "https://nightrail.rebulk.com")
      @allow_http = env_bool("NIGHTRAIL_ALLOW_HTTP", false)
      @project_root = defined?(Rails) ? Rails.root : Dir.pwd
      @detect_deploy = env_bool("NIGHTRAIL_DETECT_DEPLOY", true)
      detect_release
      ...
      @server = ENV["NIGHTRAIL_SERVER"] || ENV["KAMAL_HOST"] || Socket.gethostname
      @environment = nil # resolved lazily from Rails.env
      @sample = {
        requests: env_float("NIGHTRAIL_REQUEST_SAMPLE_RATE", 1.0),
        jobs: env_float("NIGHTRAIL_JOB_SAMPLE_RATE", 1.0),
        commands: env_float("NIGHTRAIL_COMMAND_SAMPLE_RATE", 1.0),
        scheduled_tasks: env_float("NIGHTRAIL_SCHEDULED_TASK_SAMPLE_RATE", 1.0),
        channels: env_float("NIGHTRAIL_CHANNEL_SAMPLE_RATE", 1.0),
        exceptions: env_float("NIGHTRAIL_EXCEPTION_SAMPLE_RATE", 1.0)
      }
      self.ignore = RECORD_TYPES.select { |t| env_bool("NIGHTRAIL_IGNORE_#{t.to_s.upcase}", false) }
      @log_level = (ENV["NIGHTRAIL_LOG_LEVEL"] || "info").to_sym
      @capture_request_payload = env_bool("NIGHTRAIL_CAPTURE_REQUEST_PAYLOAD", false)
      @capture_exception_source = env_bool("NIGHTRAIL_CAPTURE_EXCEPTION_SOURCE_CODE", true)
      @capture_exception_locals = env_bool("NIGHTRAIL_CAPTURE_EXCEPTION_LOCALS", false)
      @redact_headers = ENV.fetch("NIGHTRAIL_REDACT_HEADERS", "Authorization,Cookie,Set-Cookie,Proxy-Authorization,X-CSRF-Token,X-XSRF-TOKEN").split(",").map(&:strip)
      @redact_params = ENV.fetch("NIGHTRAIL_REDACT_PARAMS", "password,password_confirmation,authenticity_token,_token").split(",").map(&:strip)
      ...
      @buffer_size = env_int("NIGHTRAIL_BUFFER_SIZE", 10_000)
      ...
      @buffer_bytes = env_int("NIGHTRAIL_BUFFER_BYTES", 16 * 1024 * 1024)
      @execution_buffer_bytes = env_int("NIGHTRAIL_EXECUTION_BUFFER_BYTES", 8 * 1024 * 1024)
      @batch_bytes = env_int("NIGHTRAIL_BATCH_BYTES", 8 * 1024 * 1024)
      @backpressure = env_bool("NIGHTRAIL_BACKPRESSURE", true)
      self.backpressure_high_water = env_float("NIGHTRAIL_BACKPRESSURE_HIGH_WATER", 0.8)
      @flush_interval = env_float("NIGHTRAIL_FLUSH_INTERVAL", 2.0)
      @flush_threshold = env_int("NIGHTRAIL_FLUSH_THRESHOLD", 500)
      @connect_timeout = env_float("NIGHTRAIL_CONNECT_TIMEOUT", 1.0)
      @timeout = env_float("NIGHTRAIL_TIMEOUT", 3.0)
      @shutdown_timeout = env_float("NIGHTRAIL_SHUTDOWN_TIMEOUT", 2.0)
      @slow_query_threshold_ms = env_float("NIGHTRAIL_SLOW_QUERY_MS", 5.0)
      @n_plus_one_threshold = env_int("NIGHTRAIL_N_PLUS_ONE_THRESHOLD", 5)
      @max_view_renders_per_execution = 20
      @ignored_cache_key_prefixes = []
      @capture_default_vendor_commands = env_bool("NIGHTRAIL_CAPTURE_DEFAULT_VENDOR_COMMANDS", false)
      @capture_default_vendor_cache_keys = env_bool("NIGHTRAIL_CAPTURE_DEFAULT_VENDOR_CACHE_KEYS", false)
      @capture_framework_events = env_bool("NIGHTRAIL_CAPTURE_FRAMEWORK_EVENTS", false)
      @on_unrecoverable = nil
      @beacon_enabled = env_bool("NIGHTRAIL_BEACON", true)
      # The beacon is unauthenticated and forces Nightrail.keep! for browser
      ...
      @beacon_rate_limit = [ env_int("NIGHTRAIL_BEACON_RATE_LIMIT", 120), 0 ].max
      @debug = env_bool("NIGHTRAIL_DEBUG", false)
      # Tail-based sampling: a head-sampled-out execution is still kept when
      # it ran at least this long, raised, or Nightrail.keep! was called. nil = off.
      @tail_sample_slow_ms = ENV["NIGHTRAIL_TAIL_SAMPLE_SLOW_MS"]&.then { |v| Float(v) }
      ...
      @failure_context = env_int("NIGHTRAIL_FAILURE_CONTEXT", 0)
      @propagate_traces = env_bool("NIGHTRAIL_PROPAGATE_TRACES", true)
      @trace_propagation_hosts = ENV["NIGHTRAIL_TRACE_PROPAGATION_HOSTS"]&.split(",")&.map(&:strip)
      @health_interval = env_float("NIGHTRAIL_HEALTH_INTERVAL", 15.0)
      @capture_query_explain = env_bool("NIGHTRAIL_CAPTURE_QUERY_EXPLAIN", false)
      @explain_threshold_ms = env_float("NIGHTRAIL_EXPLAIN_THRESHOLD_MS", 100.0)
      ...
      @capture_sql_values = env_bool("NIGHTRAIL_CAPTURE_SQL_VALUES", false)
      @ignored_exceptions = ENV["NIGHTRAIL_IGNORED_EXCEPTIONS"]&.split(",")&.map(&:strip) || DEFAULT_IGNORED_EXCEPTIONS.dup
      @capture_rescued_exceptions = env_bool("NIGHTRAIL_CAPTURE_RESCUED_EXCEPTIONS", true)
      ...
      @profile_sample = env_float("NIGHTRAIL_PROFILE_SAMPLE_RATE", 0.0)
      @profile_slow_ms = ENV["NIGHTRAIL_PROFILE_SLOW_MS"]&.then { |v| Float(v) }
      @profile_interval_us = env_int("NIGHTRAIL_PROFILE_INTERVAL_US", 1_000)
      @profiler = ENV["NIGHTRAIL_PROFILER"]&.to_sym
      @capture_job_arguments = env_bool("NIGHTRAIL_CAPTURE_JOB_ARGUMENTS", false)
      @capture_job_retry_errors = env_bool("NIGHTRAIL_CAPTURE_JOB_RETRY_ERRORS", false)
      @capture_response_body_on_error = env_bool("NIGHTRAIL_CAPTURE_RESPONSE_BODY_ON_ERROR", false)
      @max_attachment_bytes = env_int("NIGHTRAIL_MAX_ATTACHMENT_BYTES", 1_048_576)
      # Release health: one `session` record per browser tab (the beacon
      # client) and per authenticated/cookied server session (Nightrail::Sessions).
      @track_sessions = env_bool("NIGHTRAIL_TRACK_SESSIONS", true)
      @session_flush_interval = env_float("NIGHTRAIL_SESSION_FLUSH_INTERVAL", 60.0)
      @session_timeout = env_float("NIGHTRAIL_SESSION_TIMEOUT", 1800.0)
      ...
      @capture_console = env_bool("NIGHTRAIL_CAPTURE_CONSOLE", false)
      @interactive_runner_paths = ENV["NIGHTRAIL_INTERACTIVE_RUNNER_PATHS"]&.split(",")&.map(&:strip) ||
        DEFAULT_INTERACTIVE_RUNNER_PATHS.dup
      @ignored_request_paths = ENV["NIGHTRAIL_IGNORED_REQUEST_PATHS"]&.split(",")&.map(&:strip)&.reject(&:empty?) ||
    ...
    def deploy=(value)
      @deploy = value
      @deploy_source = "config/initializers/nightrail.rb"
    ...
    # Nightrail.beacon_user { |request| ... }: who is behind a browser beacon.
    ...
    # Nightrail.fingerprint { |error, default| ... }: one block, called with
    # the error and the parts Nightrail would have hashed. Passing no block
    # clears it.
    ...
    # Matches Nightrail.reject_cache_keys entries and DEFAULT_VENDOR_CACHE_KEYS
    ...
    def detect_release
      ...
      @deploy_source = %w[NIGHTRAIL_DEPLOY KAMAL_VERSION].find { |key| ENV[key].to_s.strip != "" }
