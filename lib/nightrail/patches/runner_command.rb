  ...
  module Patches
    ...
    module RunnerCommand
      def perform(code_or_file = nil, *command_argv)
        return super unless Nightrail.enabled? && Nightrail.execution.nil?
      ...
      def self.instrument(code_or_file)
        ...
        exe = Nightrail.start_execution(source: :command, sample_kind: :commands, preview: "rails runner #{preview}")
        ...
        begin
          ...
          exit_code = 1
          Nightrail::Subscribers::Exceptions.capture(e, handled: false, severity: :error, source: "application.runner")
          raise
        ensure
          exe.finish_stages
          fields = {
          ...
          Nightrail.finish_execution(:command, **fields)
          Nightrail.flush
        end
      end
      ...
      def self.scratch?(argument)
        ...
        Nightrail.config.interactive_runner_paths.any? { |directory| path.start_with?(directory) }
      end
    end
  end
end
