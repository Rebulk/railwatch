...
module Nightrail
  ...
  class Execution
    ...
    # profiler (Nightrail.start_profile): the backend handle, plus whether the
    ...
    # the key of the session this request belongs to (Nightrail::Sessions),
    ...
    # Set only by Nightrail::Patches::RunnerCommand, for a `rails runner` an
    ...
    def initialize(source:, sampled:, trace_id: nil, parent_id: nil, preview: nil)
      ...
      @tail_buffering = !Nightrail.config.tail_sample_slow_ms.nil?
      ...
      # exception promotes them (Nightrail.tail_keep?). Skipped when tail
      ...
      # it has always been.
      @failure_context = !sampled && !@tail_buffering && Nightrail.config.failure_context.positive?
      @tail_buffering ||= @failure_context
      @record_limit = @failure_context ? Nightrail.config.failure_context : MAX_RECORDS
      @byte_limit = Nightrail.config.execution_buffer_bytes
    ...
    # decision (Nightrail.keep!), buffering child records from here on.
    def keep!
      @keep = true
      @tail_buffering = true
    ...
    # decision made late (route-level nightrail_sample, dont_sample) still
    ...
    def buffer(record)
      ...
      # loss is counted onto the parent's batch (Nightrail.finish_execution).
      if @failure_context
    ...
    # nested under Nightrail's (activerecord-tenanted's TenantSelector), an
