...
module Nightrail
  ...
  class Reporter
    ...
    def shutdown
      ...
      Nightrail.debug { "shutdown flush failed: #{e.class}: #{e.message}" }
      Nightrail.notify_unrecoverable(e)
    end
    ...
    def arm_thread
      @mutex.synchronize do
        ...
        @thread.name = "nightrail-reporter"
    ...
    def run
      loop do
        ...
        begin
          ...
          Nightrail.debug { "flush error: #{e.class}: #{e.message}" }
          Nightrail.notify_unrecoverable(e)
        end
      end
    end
    ...
    def deliver_buffer
      ...
      unless batch.prepared
        deliverable = Nightrail.run_before_ingest(batch.records)
      ...
      if result.ok
        ...
        Nightrail.debug { "ingest rejected #{result.rejected} of #{deliverable.size} records" } if result.rejected.to_i.positive?
        delivery_succeeded
    ...
    def retain(batch, result)
      @mutex.synchronize do
        ...
        if @retry_attempt > MAX_RETRY_ATTEMPTS
          ...
          @retry_attempt = 0
          @retry_at = nil
          Nightrail.debug { "gave up on a batch of #{batch.records.size} records after #{MAX_RETRY_ATTEMPTS} retries (#{result.error || result.status}); dropped and counted" }
          next
        end
        @retry_batch = batch
    ...
    def discard_unauthorized
      batch = drain_into_flight
      delivery_succeeded
      Nightrail.debug { "transport unauthorized; discarded #{batch.records.size} records (dropped #{batch.dropped})" } unless batch.records.empty?
      nil
    end
    ...
    def delivery_rejected(batch, result)
      ...
      Nightrail.notify_unrecoverable(
        DeliveryError.new("Nightrail ingest permanently rejected #{batch.records.size} records: #{detail}",
    ...
    def notify_unsent(reason)
      ...
      Nightrail.notify_unrecoverable(
        DeliveryError.new("Nightrail #{reason} with #{records} unsent records retained in memory (#{bytes} bytes)",
