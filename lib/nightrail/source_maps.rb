...
module Nightrail
  ...
  class SourceMaps
    ...
    def upload(directory: "public", delete: false)
      raise ArgumentError, "NIGHTRAIL_TOKEN is not set" if @config.token.to_s.empty?
      raise ArgumentError, "NIGHTRAIL_DEPLOY (or KAMAL_VERSION) is not set" if @config.deploy.to_s.empty?
      raise ArgumentError, "plain HTTP ingest is disabled; use HTTPS or set NIGHTRAIL_ALLOW_HTTP=true" unless @config.ingest_url_allowed?
    ...
    def upload_file(filename, data)
      ...
      request["X-Nightrail-Deploy"] = @config.deploy
      request["X-Nightrail-Filename"] = filename
      request.body = data
