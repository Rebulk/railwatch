...
module Nightrail
  ...
  module Profiler
    ...
    class << self
      ...
      # loads wins.
      def backend
        wanted = Nightrail.config.profiler
      ...
      def start(mode: :wall)
        ...
        @lock.synchronize do
          ...
          handle = Handle.new(backend, mode, Nightrail.config.profile_interval_us,
        ...
        Nightrail.debug { "profiler start failed: #{e.class}: #{e.message}" }
        nil
      end
      ...
      def stop
        ...
        Nightrail.debug { "profiler stop failed: #{e.class}: #{e.message}" }
        nil
      end
