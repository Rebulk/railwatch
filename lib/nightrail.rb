...
require "nightrail/version"
require "nightrail/clock"
require "nightrail/release_detector"
require "nightrail/configuration"
require "nightrail/secret_safety"
require "nightrail/execution"
require "nightrail/current"
require "nightrail/record"
require "nightrail/buffer"
require "nightrail/transport/http"
require "nightrail/reporter"
require "nightrail/sampler"
require "nightrail/redactor"
require "nightrail/sql_normalizer"
require "nightrail/backtrace"
require "nightrail/context"
require "nightrail/profiler"
require "nightrail/attachments"
# Nightrail::Faraday subclasses ::Faraday::Middleware at load time, so it can't
...
# order, so nightrail itself is often required *before* an app's own `gem
# "faraday"` line further down the Gemfile. lib/nightrail/engine.rb requires it
...
module Nightrail
  ...
  # process boot as Nightrail can observe. `process` records measure boot_seconds
  ...
  class << self
    ...
    def report(error, handled: true, severity: nil, context: {}, attachments: nil, fingerprint: nil)
      rec = Subscribers::Exceptions.capture(error, handled: handled, severity: severity || (handled ? :warning : :error),
                                            context: context, source: "nightrail.manual", fingerprint: fingerprint)
    ...
    # block's value. A no-op wrapper (still yields) when Nightrail is disabled
    # or nothing is recording.
    #
    #   Nightrail.span("pdf.render", pages: 12) { renderer.call }
    ...
    # Internal diagnostics, only when NIGHTRAIL_DEBUG=1. Goes to stderr rather
    # than Rails.logger so it can never be captured as an app log record.
    def debug
      return unless config.debug
      warn("[nightrail] #{yield}")
    end
    ...
    def tail_keep?(exe)
      ...
      # withheld from an interactive runner, raised inside Nightrail.ignore, or
    ...
    def start_profile(exe)
      # An app's test suite inherits its NIGHTRAIL_* environment, and starting
    ...
    # config.redactors/rejectors default their Hash on write (Nightrail.redact_*)
...
require "nightrail/subscribers"
require "nightrail/engine" if defined?(Rails::Engine)
