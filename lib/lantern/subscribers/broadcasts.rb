...
module Lantern
  module Subscribers
    ...
    module Broadcasts
      ...
      EXECUTION_KEY = :__lantern_channel_execution
      ...
      class ActionExecution
        def start(_name, _id, payload)
          ...
          exe = Lantern.start_execution(source: :channel_action, sample_kind: :channels,
          ...
          Lantern.debug { "perform_action.action_cable start subscriber raised #{e.class}: #{e.message}" }
        ...
        def finish(_name, _id, payload)
          ...
          Lantern.record(:broadcast, group: Record.group_hash(channel, action),
                         timestamp: exe.started_at, kind: "perform_action", channel: channel,
                         action: action, duration: exe.duration, failed: error ? true : false)
          Lantern.finish_execution(:channel_action, group: Record.group_hash(channel, action),
                                   channel: channel, action: action,
                                   status: error ? "failed" : "processed", failed: error ? true : false)
        rescue StandardError => e
          Lantern.debug { "perform_action.action_cable finish subscriber raised #{e.class}: #{e.message}" }
          Lantern.notify_unrecoverable(e)
        ensure
          ...
          Lantern::Current.execution = exe.parent_execution if exe && Lantern.execution.equal?(exe)
        end
      end
