...
module Lantern
  ...
  module Profiler
    ...
    class << self
      ...
      # loads wins.
      def backend
        wanted = Lantern.config.profiler
      ...
      def start(mode: :wall)
        ...
        @lock.synchronize do
          ...
          handle = Handle.new(backend, mode, Lantern.config.profile_interval_us,
        ...
        Lantern.debug { "profiler start failed: #{e.class}: #{e.message}" }
        nil
      end
      ...
      def stop(expected_handle = nil)
        ...
        Lantern.debug { "profiler stop failed: #{e.class}: #{e.message}" }
        nil
      end
