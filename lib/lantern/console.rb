...
module Lantern
  ...
  # production types typos, and a typo is not an issue. Lantern subscribes to
  ...
  # `config.capture_console = true` (or LANTERN_CAPTURE_CONSOLE=1) for the
  # rare "trace what I'm about to do in here" session.
  #
  # A `rails runner` script is NOT this: it is a deployed execution and keeps
  # reporting -- see Lantern::Patches::RunnerCommand for where that line is.
  module Console
    module_function
    ...
    def quiet?
      detected? && !Lantern.config.capture_console
    end
    ...
    def silence!
      return false unless quiet? && Lantern.config.enabled

      Lantern.debug { "console detected -- capturing nothing in this process (set config.capture_console = true, or LANTERN_CAPTURE_CONSOLE=1, to capture a console session)" }
