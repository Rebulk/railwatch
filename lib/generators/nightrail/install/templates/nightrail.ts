// Nightrail browser client for Inertia. Reports each visit's duration,
// component, and prop payload size to /nightrail/beacon so the platform can
...
export interface NightrailOptions {
  ...
  denyUrls?: RegExp[]
  // The tenant the user is looking at. The beacon posts to /nightrail/beacon,
...
const SESSION_ID_KEY = "nightrail.session"
const SESSION_STARTED_KEY = "nightrail.session.at"
...
// session (Nightrail::Sessions) join the browser side under one id.
function startSession() {
  try {
    ...
    document.cookie = `nightrail_session=${id}; path=/; SameSite=Lax`
...
// React 19's root error options, for `createRoot(el, nightrailRootOptions())`.
//
...
export function nightrailRootOptions(): {
...
export function startNightrail(options: NightrailOptions = {}) {
