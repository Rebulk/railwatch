...
Security fixes are released on the latest published Nightrail version. Upgrade
...
the configured Nightrail Cloud ingest URL. It does not accept commands from
Nightrail Cloud and never writes to the application's database. Its one inbound
route is the unauthenticated browser beacon mounted under `/nightrail/beacon`;
...
`NIGHTRAIL_CAPTURE_EXCEPTION_SOURCE_CODE=false`. Request payloads, exception
