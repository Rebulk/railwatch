...
RSpec.describe "tail sampling", type: :request do
  ...
  after { Nightrail.config.tail_sample_slow_ms = nil }
  ...
  it "ships the whole tree of a head-sampled-out request that ran at least tail_sample_slow_ms" do
    ...
    Nightrail.config.tail_sample_slow_ms = 0.0 # every request is "slow"
    get "/widgets"

    expect(nightrail_records(:query)).not_to be_empty
    expect(nightrail_records(:log)).not_to be_empty
    expect(nightrail_records(:request).sole[:tail_sampled]).to be(true)
  end

  it "ships nothing for a head-sampled-out request that finished faster than tail_sample_slow_ms" do
    Nightrail.config.sample[:requests] = 0.0
    Nightrail.config.tail_sample_slow_ms = 60_000.0
    get "/widgets"
  ...
  it "leaves a normally sampled-in request unmarked, since it was never a tail decision" do
    Nightrail.config.tail_sample_slow_ms = 0.0
    get "/widgets"
  ...
  it "ships the children of a sampled-out execution that raised, once tail sampling is on" do
    require "rake"
    Rake::Task.define_task(:nightrail_tail_boom) do
      Widget.count
      raise ArgumentError, "kaboom"
    end
    Nightrail.config.sample[:commands] = 0.0
    Nightrail.config.tail_sample_slow_ms = 60_000.0 # not slow enough to qualify on duration

    expect { Rake::Task[:nightrail_tail_boom].execute }.to raise_error(ArgumentError)
  ...
  it "still ships only the parent for a sampled-out execution that raised when tail sampling is off" do
    require "rake"
    Rake::Task.define_task(:nightrail_no_tail_boom) do
      Widget.count
      raise ArgumentError, "kaboom"
    end
    Nightrail.config.sample[:commands] = 0.0

    expect { Rake::Task[:nightrail_no_tail_boom].execute }.to raise_error(ArgumentError)

    expect(nightrail_records(:query)).to be_empty
    expect(nightrail_records(:command).sole).not_to have_key(:tail_sampled)
  end

  describe "Nightrail.keep!" do
    it "ships the whole tree of a sampled-out execution even with tail sampling off" do
      ...
      Nightrail.keep!
      Nightrail.record(:log, level: "info", message: "kept", tags: [], context: "{}")
      finish_command

      expect(nightrail_records(:log).sole[:message]).to eq("kept")
      expect(nightrail_records(:command).sole[:tail_sampled]).to be(true)
    end
    ...
    it "cannot resurrect records made before it, which were never buffered" do
      Nightrail.config.sample[:commands] = 0.0
      Nightrail.start_execution(source: :command, sample_kind: :commands)
      Nightrail.record(:log, level: "info", message: "before", tags: [], context: "{}")
      Nightrail.keep!
      Nightrail.record(:log, level: "info", message: "after", tags: [], context: "{}")
      finish_command

      expect(nightrail_records(:log).map { |l| l[:message] }).to eq([ "after" ])
    end

    it "does nothing outside an execution" do
      expect { Nightrail.keep! }.not_to raise_error
    end
  end
  ...
  describe "profile_slow_ms" do
    before do
      ...
      Nightrail.config.profile_sample = 0.5
      Nightrail.config.profile_interval_us = 500
    end

    after do
      Nightrail.config.profile_sample = 0.0
      Nightrail.config.profile_slow_ms = nil
      Nightrail.config.profile_interval_us = 1_000
      Nightrail::Profiler.reset!
    end
    ...
    it "ships the profile of a tail-buffering request that ran at least profile_slow_ms" do
      ...
      Nightrail.config.profile_slow_ms = 0.0 # every request is "slow"
      get "/widgets"

      expect(nightrail_records(:profile).sole[:profiler]).to eq("vernier")
      expect(nightrail_records(:request).sole[:profiled]).to be(true)
    end

    it "throws the profile away when the request finished faster than profile_slow_ms" do
      Nightrail.config.tail_sample_slow_ms = 500.0
      Nightrail.config.profile_slow_ms = 60_000.0
      get "/widgets"

      expect(nightrail_records(:profile)).to be_empty
      expect(nightrail_records(:request).sole).not_to have_key(:profiled)
    end

    it "never starts a profile when tail sampling is off" do
      Nightrail.config.profile_slow_ms = 0.0
  ...
  describe "Execution#recording?" do
    it "is unchanged for a sampled-out execution when tail sampling is off" do
      exe = Nightrail::Execution.new(source: :request, sampled: false)
    ...
    it "stays paused inside Nightrail.ignore even when buffering for the tail" do
      Nightrail.config.tail_sample_slow_ms = 500.0
      exe = Nightrail::Execution.new(source: :request, sampled: false)
      exe.paused_depth += 1
