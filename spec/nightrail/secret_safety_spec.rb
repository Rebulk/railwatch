...
RSpec.describe Nightrail::SecretSafety do
  ...
  it "distinguishes ignored, tracked, and ordinary dotenv paths" do
    Dir.mktmpdir do |root|
      ...
      File.write(File.join(root, ".env"), "NIGHTRAIL_TOKEN=lt_a_real_secret\n")
  ...
  it "returns only tracked paths containing plaintext Nightrail tokens" do
    Dir.mktmpdir do |root|
      system("git", "init", "-q", root)
      FileUtils.mkdir_p(File.join(root, "config/initializers"))
      File.write(File.join(root, ".env"), "NIGHTRAIL_TOKEN=lt_not_committed\n")
      File.write(File.join(root, "config/initializers/nightrail.rb"), 'c.token = "lt_committed_secret"\n')
      system("git", "-C", root, "add", "config/initializers/nightrail.rb")

      expect(described_class.tracked_plaintext_token_files(root: root))
        .to eq([ "config/initializers/nightrail.rb" ])
    end
  end
end
