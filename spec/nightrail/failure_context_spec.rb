...
RSpec.describe "failure context", type: :request do
  ...
  after do
    Nightrail.config.failure_context = 0
    Nightrail.config.tail_sample_slow_ms = nil
  end

  def start_command
    Nightrail.config.sample[:commands] = 0.0
  ...
  def log(message)
    Nightrail.record(:log, level: "info", message: message, tags: [], context: "{}")
  end

  describe "a head-sampled-out request" do
    before do
      Nightrail.config.sample[:requests] = 0.0
      Nightrail.config.failure_context = 200
    end
    ...
    it "ships the child records that led up to an unhandled exception, with the exception and the parent" do
      ...
      expect(nightrail_records(:request).sole).to include(status_code: 500, tail_sampled: true)
    end
    ...
    it "still lets a late Nightrail.sample(1.0) ship the whole execution" do
    ...
    it "keeps shipping only the lone parent once failure_context is back off" do
      ...
      expect(nightrail_records(:request).sole).not_to have_key(:tail_sampled)
    end
  end

  describe "a head-sampled-out Active Job attempt" do
    before do
      Nightrail.config.sample[:jobs] = 0.0
      Nightrail.config.failure_context = 200
    end
    ...
    it "ships its queries and logs when the job raises" do
      ...
      expect(nightrail_records(:query)).not_to be_empty
      expect(nightrail_records(:exception).sole[:handled]).to be(false)
      expect(nightrail_records(:job_attempt).sole).to include(status: "failed", tail_sampled: true)
    end
  ...
  describe "a head-sampled-out scheduled task" do
    ...
    before do
      Nightrail.config.sample[:scheduled_tasks] = 0.0
      Nightrail.config.failure_context = 200
    end
    ...
    def register_recurring(key, job)
      ...
      Nightrail::Subscribers::Jobs.refresh_recurring_tasks!
    end
    ...
    it "ships its child records when the task raises" do
      ...
      expect(nightrail_records(:query)).not_to be_empty
      expect(nightrail_records(:scheduled_task).sole).to include(task_key: "widget_failure_context", tail_sampled: true)
    end
  ...
  describe "a head-sampled-out command" do
    before { Nightrail.config.failure_context = 200 }

    it "ships the queries a failing rake task ran before it raised" do
      Rake::Task.define_task(:nightrail_failure_context_boom) do
        Widget.count
        raise ArgumentError, "kaboom"
      end
      Nightrail.config.sample[:commands] = 0.0

      expect { Rake::Task[:nightrail_failure_context_boom].execute }.to raise_error(ArgumentError)

      expect(nightrail_records(:query)).not_to be_empty
      expect(nightrail_records(:exception).sole[:handled]).to be(false)
      expect(nightrail_records(:command).sole[:tail_sampled]).to be(true)
    end

    it "ships nothing for a rake task that completes" do
      Rake::Task.define_task(:nightrail_failure_context_ok) { Widget.count }
      Nightrail.config.sample[:commands] = 0.0

      Rake::Task[:nightrail_failure_context_ok].execute
    ...
    it "never promotes the ring for an ignored exception class" do
      start_command
      log("before")
      Nightrail.report(ActiveRecord::RecordNotFound.new("Couldn't find Widget"), handled: false)
      finish_command

      expect(nightrail_records).to be_empty
    end

    it "never promotes the ring for an exception reported inside Nightrail.ignore, shipping only the parent" do
      start_command
      log("before")
      Nightrail.ignore { Nightrail.report(RuntimeError.new("paused"), handled: false) }
      finish_command

      expect(nightrail_records(:log)).to be_empty
      expect(nightrail_records(:exception)).to be_empty
      expect(nightrail_records(:command).sole).not_to have_key(:tail_sampled)
    end
    ...
    it "never promotes the ring for an interactive `rails runner`, whose exceptions are the operator's" do
      ...
      log("before")
      Nightrail.report(RuntimeError.new("typo"), handled: false)
      finish_command
  ...
  describe "bounded memory" do
    ...
    it "accounts the overflow onto the batch's dropped count when the ring is promoted" do
      Nightrail.config.failure_context = 10
      start_command
      25.times { |i| log("line #{i}") }
      Nightrail.report(RuntimeError.new("boom"), handled: false)
      finish_command

      expect(Nightrail.reporter.buffer.dropped).to eq(15)
      expect(nightrail_records(:log).map { |r| r[:message] }).to eq((15..24).map { |i| "line #{i}" })
    end

    it "keeps every concurrent execution's ring bounded and separate" do
      Nightrail.config.failure_context = 50
      Nightrail.config.sample[:jobs] = 0.0

      executions = 4.times.map do |t|
        Thread.new do
          exe = Nightrail.start_execution(source: :job, sample_kind: :jobs)
  ...
  describe "Execution" do
    def sampled_out_execution
      Nightrail::Execution.new(source: :request, sampled: false)
    end
    ...
    it "records nothing for a sampled-out execution while failure_context is 0, which is the default" do
      ...
      expect(Nightrail.config.failure_context).to eq(0)
    ...
    it "never rings a head-sampled-in execution" do
      Nightrail.config.failure_context = 200
      exe = Nightrail::Execution.new(source: :request, sampled: true)
    ...
    it "leaves tail sampling's larger buffer in charge when both are configured" do
      Nightrail.config.failure_context = 2
      Nightrail.config.tail_sample_slow_ms = 500.0
      exe = sampled_out_execution
    ...
    it "stops ringing once Nightrail.keep! takes over the execution" do
      Nightrail.config.failure_context = 2
      exe = sampled_out_execution
    ...
    it "stays paused inside Nightrail.ignore even while ringing" do
      Nightrail.config.failure_context = 200
      exe = sampled_out_execution
      exe.paused_depth += 1
