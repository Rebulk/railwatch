...
RSpec.describe Nightrail::Subscribers::Jobs do
  around do |example|
    Nightrail.config.capture_job_retry_errors = false
    example.run
  ensure
    Nightrail.config.capture_job_retry_errors = false
  end
  ...
  it "keeps the retry log but does not capture the retry error by default" do
    ...
    retry_log = nightrail_records(:log).find { |log| log[:tags] == [ "active_job", "retry" ] }
  ...
  it "captures a retry error as a handled warning on the current job attempt" do
    Nightrail.config.capture_job_retry_errors = true
  ...
  it "captures the retry once and the exhausted retry once, with their respective dispositions" do
    ...
    attempts = nightrail_records(:job_attempt)
    exceptions = nightrail_records(:exception)
