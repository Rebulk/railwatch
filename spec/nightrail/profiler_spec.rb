...
RSpec.describe Nightrail::Profiler do
  after do
  ...
  def churn(milliseconds)
    deadline = Nightrail::Clock.monotonic + (milliseconds / 1000.0)
    n = 0
    n += 1 while Nightrail::Clock.monotonic < deadline
    n
  end
  ...
  describe "backend selection" do
    ...
    it "has no backend when config.profiler names one that isn't installed" do
      Nightrail.config.profiler = :not_a_profiler
  ...
  describe "start/stop" do
    %i[vernier stackprof].each do |backend|
      context "with #{backend}" do
        before do
          Nightrail.config.profiler = backend
    ...
    it "degrades to nil when the backend raises on stop" do
      ...
      # The stub, not Nightrail, is what left vernier collecting: put the real
  ...
  describe ".collapse" do
    ...
    it "formats each frame as `Class#method (path:line)`" do
      result = stackprof_result([ [ [ "Widget#save" ], 3 ] ], files: { "Widget#save" => "#{Nightrail::Backtrace.app_root}app/models/widget.rb" })
    ...
    it "caps the text at 4 MiB, dropping the least frequent stacks" do
      ...
      expect(collapsed.bytesize).to be <= Nightrail::Profiler::MAX_COLLAPSED_BYTES
