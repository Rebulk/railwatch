...
RSpec.describe "exception fingerprinting" do
  subject(:exceptions) { Nightrail::Subscribers::Exceptions }

  # The resolver lives on the shared config for the whole process; calling
  # Nightrail.fingerprint with no block clears it again.
  after { Nightrail.fingerprint }
  ...
  def report(error, **options)
    Nightrail.report(error, handled: true, **options)
    nightrail_records(:exception).last
  end
  ...
  describe "the default fingerprint" do
    it "ships on the record as its four parts plus the source it came from" do
      ...
      expect(record[:_group]).to eq(Nightrail::Record.group_hash(*record[:fingerprint]))
    end
  ...
  describe "precedence" do
    it "prefers an explicit fingerprint: over everything else" do
      Nightrail.fingerprint { |_error, _default| [ "from-resolver" ] }
      error = raised("boom")
      def error.nightrail_fingerprint = [ "from-error" ]
    ...
    it "prefers the error's own #nightrail_fingerprint over the resolver" do
      Nightrail.fingerprint { |_error, _default| [ "from-resolver" ] }
      error = raised("boom")
      def error.nightrail_fingerprint = [ "payment", "adyen" ]
    ...
    it "uses the resolver when the error has no fingerprint of its own" do
      Nightrail.fingerprint { |error, _default| [ "class", error.class.name ] }
    ...
    it "falls back to the default when the resolver returns nil or an empty array" do
      Nightrail.fingerprint { |_error, _default| nil }
      expect(report(raised("boom"))[:fingerprint_source]).to eq("default")

      Nightrail.fingerprint { |_error, _default| [] }
      expect(report(raised("boom"))[:fingerprint_source]).to eq("default")
    end

    it "falls back to the default when the resolver raises" do
      Nightrail.fingerprint { |_error, _default| raise "resolver is broken" }
  ...
  describe "the :default splice" do
    it "expands to the parts Nightrail would have hashed, in place" do
      Nightrail.fingerprint { |_error, _default| [ "tenant-7", :default ] }
    ...
    it "gives two errors that would group apart the same group once spliced under a shared key" do
      Nightrail.fingerprint { |_error, _default| [ "checkout" ] }
  ...
  describe "part limits" do
    it "stringifies parts, drops the empty ones, and caps at 10 parts of 200 characters" do
      Nightrail.fingerprint { |_error, _default| [ :billing, 42, nil, "", "x" * 300, *Array.new(20, "pad") ] }
  ...
  describe "attachments" do
    it "files an attachment against the issue the custom fingerprint chose" do
      Nightrail.fingerprint { |_error, _default| [ "payments", "gateway-timeout" ] }
      error = raised("boom")

      record = report(error)
      Nightrail.attach("payload.json", '{"order":1}', exception: error)

      expect(nightrail_records(:attachment).last[:exception_group_hash]).to eq(record[:_group])
    end

    it "files an attachment against the issue an explicit Nightrail.report fingerprint chose" do
      error = raised("boom")
      record = report(error, fingerprint: [ "billing", "stripe" ])
