...
RSpec.describe "span record" do
  ...
  it "ships as a v1 span with the name, its group hash, and no attributes when none were given" do
    ...
    expect(span[:_group]).to eq(Nightrail::Record.group_hash("pdf.render"))
  ...
  it "belongs to the execution it ran inside" do
    Nightrail.start_execution(source: :command, sample_kind: :commands)
    Nightrail.span("pdf.render") { :done }
    Nightrail.finish_execution(:command, group: "g", class: "Rake::Task", name: "demo",
                             command: "rake demo", exit_code: 0)
