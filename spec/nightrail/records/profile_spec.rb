...
RSpec.describe "profile record", type: :request do
  before do
    ...
    Nightrail.config.profile_interval_us = 200
  ...
  it "ships one v1 profile for a request when every request is profiled" do
    Nightrail.config.profile_sample = 1.0
    get "/widgets"

    profile = nightrail_records(:profile).sole
  ...
  it "carries the request's envelope, so the profile joins its execution" do
    ...
    profile = nightrail_records(:profile).sole
    expect(profile[:execution_id]).to eq(nightrail_records(:request).sole[:execution_id])
  ...
  it "ships the collapsed stacks gzipped and base64-encoded, with the uncompressed size" do
    ...
    expect(stacks.bytesize).to be <= Nightrail::Profiler::MAX_COLLAPSED_BYTES
  ...
  it "names the dummy app's own controller frame in the collapsed stacks" do
    ...
    expect(stacks_of(nightrail_records(:profile).sole))
  ...
  it "profiles with stackprof when config.profiler pins it" do
    Nightrail.config.profile_sample = 1.0
    Nightrail.config.profiler = :stackprof
    get "/widgets"
  ...
  it "profiles only the outer execution when one execution nests inside another" do
    ...
    inner = Nightrail.start_execution(source: :job, sample_kind: :jobs)

    expect(outer.profiler_handle).not_to be_nil
    expect(inner.profiler_handle).to be_nil
    expect(Nightrail::Profiler.skipped).to eq(1)

    Nightrail.finish_execution(:job_attempt, group: "g", name: "InnerJob", queue: "default")
    Nightrail.finish_execution(:command, group: "g", class: "Rake::Task", name: "outer",
                             command: "rake outer", exit_code: 0)

    expect(nightrail_records(:profile).sole[:execution_id]).to eq(outer.id)
  end
end
