...
RSpec.describe "user record", type: :request do
  ...
  it "resolves id, name, and email from Current.user by default" do
    ...
    user = nightrail_records(:user).sole
  ...
  it "ships only one user record across two requests from the same user within the hour" do
    ...
    expect(nightrail_records(:user).size).to eq(1)
  end
  ...
  it "does not cache a user whose first resolution is sampled out" do
    ...
    exe.sampled = false
    Nightrail::Subscribers::Users.remember(details)
    finish!
    ...
    expect(nightrail_records(:user).map { |record| record[:id] }).to eq([ "sampled-user" ])
  end
  ...
  it "does not cache a buffered user when controller-level sampling later drops the execution" do
    ...
    expect(nightrail_records(:user).map { |record| record[:id] }).to eq([ "late-sampled-user" ])
  end
  ...
  it "does not cache a user whose first resolution is paused" do
    ...
    expect(nightrail_records(:user).map { |record| record[:id] }).to eq([ "paused-user" ])
  end
  ...
  it "emits the same user again after fork state is reset" do
    ...
    Nightrail::Subscribers::Users.restart_after_fork!
    Nightrail.start_execution(source: :command, sample_kind: :commands)
    Nightrail::Subscribers::Users.remember(details)
    finish!

    expect(nightrail_records(:user).map { |record| record[:id] }).to eq([ "fork-user", "fork-user" ])
  end

  it "uses config.user's custom resolver block instead of the id/name/email default" do
    Nightrail.config.user { |u| { id: "custom-#{u.id}", name: "Custom #{u.name}", email: nil } }
    ...
    Nightrail.config.instance_variable_set(:@user_resolver, nil)
  end
  ...
  it "keeps the same numeric raw id distinct across two tenants that bind late" do
    ...
    expect(nightrail_records(:user).map { |record| [ record[:id], record[:tenant] ] })
      .to contain_exactly([ "acme:1", "acme" ], [ "beta:1", "beta" ])
    expect(nightrail_records(:command).map { |record| record[:user] })
  ...
  it "ships one entity, not one per request, when the same tenant binds late twice" do
    ...
    expect(nightrail_records(:user).map { |record| record[:id] }).to eq([ "acme:1" ])
    expect(nightrail_records(:command).map { |record| record[:user] }).to eq([ "acme:1", "acme:1" ])
  ensure
    Current.user = nil
  end
end
