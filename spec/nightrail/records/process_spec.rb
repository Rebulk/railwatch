...
RSpec.describe "process record" do
  it "captures pid, role, ruby/rails/nightrail versions, adapters, cache store, and boot_seconds" do
    Nightrail::Subscribers::ProcessInfo.install!(Rails.application)
    Nightrail::Subscribers::ProcessInfo.record!

    proc_rec = nightrail_records(:process).sole
    ...
    expect(proc_rec[:nightrail_version]).to eq(Nightrail::VERSION)
  ...
  it "names the adapters from the app's configuration" do
    ...
    expect(Nightrail::Subscribers::ProcessInfo.database_adapter).to eq("sqlite3")
    expect(Nightrail::Subscribers::ProcessInfo.configured_queue_adapter).to eq("test")
  end
  ...
  it "resolves a url-only database.yml, a DATABASE_URL-only app, and a self-named queue adapter" do
    ...
    Nightrail::Subscribers::ProcessInfo.install!(app)
    expect(Nightrail::Subscribers::ProcessInfo.database_adapter).to eq("postgresql")

    url_only = Struct.new(:config).new(Struct.new(:database_configuration, :active_job).new({}, Struct.new(:queue_adapter).new(nil)))
    Nightrail::Subscribers::ProcessInfo.install!(url_only)
    previous_url = ENV["DATABASE_URL"]
    ENV["DATABASE_URL"] = "mysql2://u@h/d"
    begin
      expect(Nightrail::Subscribers::ProcessInfo.database_adapter).to eq("mysql2")
    ensure
    ...
    Nightrail::Subscribers::ProcessInfo.install!(with_adapter)
    expect(Nightrail::Subscribers::ProcessInfo.configured_queue_adapter).to eq("acme_queue")
  ensure
    Nightrail::Subscribers::ProcessInfo.install!(Rails.application)
  end
  ...
  it "reports the effective queue adapter once Active Job is loaded, even when it was set on ActiveJob::Base directly" do
    ...
    expect(Nightrail::Subscribers::ProcessInfo.queue_adapter).to eq("inline")
    expect(Nightrail::Subscribers::ProcessInfo.configured_queue_adapter).to eq("test")
  ensure
  ...
  describe ".role" do
    ...
    it "is web when Puma is defined" do
      ...
      expect(Nightrail::Subscribers::ProcessInfo.role).to eq("web")
    end

    it "is worker when SolidQueue is defined and $PROGRAM_NAME includes jobs" do
      stub_const("SolidQueue", Module.new)
      with_program_name("bin/jobs") { expect(Nightrail::Subscribers::ProcessInfo.role).to eq("worker") }
    end
    ...
    it "is worker for `rails solid_queue:start` even though Puma is loaded" do
      ...
      with_program_name("bin/rails") { expect(Nightrail::Subscribers::ProcessInfo.role).to eq("worker") }
    end

    it "is console when Rails::Console is defined" do
      stub_const("Rails::Console", Class.new)
      expect(Nightrail::Subscribers::ProcessInfo.role).to eq("console")
    end

    it "is command when $PROGRAM_NAME ends with rake" do
      with_program_name("/usr/bin/rake") { expect(Nightrail::Subscribers::ProcessInfo.role).to eq("command") }
    end

    it "falls back to process when nothing else matches" do
      with_program_name("irb") { expect(Nightrail::Subscribers::ProcessInfo.role).to eq("process") }
    end
