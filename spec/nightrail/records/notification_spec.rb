...
# Nightrail::Subscribers::Notifications.install! never runs its
...
RSpec.describe "notification record" do
  before(:context) { Nightrail::Subscribers::Notifications.install!(Rails.application) }
  ...
  it "derives channel from the Noticed delivery class" do
    ...
    finish!
    expect(nightrail_records(:notification).sole[:channel]).to eq("test")
  end
  ...
  it "captures the notifier class, delivery_method, duration, and failed false for a clean delivery" do
    ...
    rec = nightrail_records(:notification).sole
  ...
  it "reports failed true when the delivery job raises" do
    ...
    expect(nightrail_records(:notification).sole[:failed]).to be(true)
  end
  ...
  it "never ships a notification record for an ordinary (non-Noticed::) job" do
    ...
    expect(nightrail_records(:notification)).to be_empty
  end
end
