...
RSpec.describe "exception record", type: :request do
  it "captures class, message, handled, severity, source, file, line, and frames for an unhandled error" do
    ...
    # before the error propagates up to Nightrail's outer middleware rescue,
    # and Nightrail's dedupe (Exceptions.seen?) means whichever capture fires
  ...
  it "groups on the default fingerprint -- class, top in-app frame, normalized message -- and says so" do
    ...
    expect(ex[:_group]).to eq(Nightrail::Record.group_hash(*ex[:fingerprint]))
  end
  ...
  it "does not let a sampled-out handled capture suppress a later unhandled capture" do
    ...
    Nightrail.report(error, handled: true)
    Nightrail.report(error, handled: false)
    Nightrail.finish_execution

    expect(nightrail_records(:exception).sole).to include(message: "becomes fatal", handled: false)
  end
  ...
  it "does not let a paused handled capture suppress a later unhandled capture" do
    ...
    expect(nightrail_records(:exception).sole).to include(message: "visible after resume", handled: false)
  end
  ...
  it "captures handled and unhandled observations as distinct dispositions" do
    ...
    expect(nightrail_records(:exception).map { |record| record[:handled] }).to contain_exactly(true, false)
  end
  ...
  it "captures a reused exception object once in each execution" do
    ...
    expect(nightrail_records(:exception).map { |record| record[:message] }).to eq([ "reused", "reused" ])
  end

  it "keeps an outer execution deduped across many nested executions" do
    error = ArgumentError.new("nested reuse")
    outer = Nightrail.start_execution(source: :command, sample_kind: :commands)
    Nightrail.report(error, handled: true)

    5.times do
      Nightrail.start_execution(source: :job, sample_kind: :jobs)
      Nightrail.report(error, handled: true)
      Nightrail.finish_execution
    end
    ...
    outer_records = nightrail_records(:exception).select { |record| record[:execution_id] == outer.id }
  ...
  describe "ignored_exceptions" do
    ...
    def with_ignored(list)
      ...
      Nightrail.config.ignored_exceptions = list
      yield
    ensure
    ...
    def report_missing_widget
      Nightrail.start_execution(source: :job, sample_kind: :jobs)
      Nightrail.report(ActiveRecord::RecordNotFound.new("Couldn't find Widget with 'id'=999"), handled: true)
      Nightrail.finish_execution
    end

    it "drops ActiveRecord::RecordNotFound, which the default list carries over from Sentry" do
      expect(Nightrail.config.ignored_exceptions).to include("ActiveRecord::RecordNotFound")
    ...
    it "captures that same error once it is removed from the list" do
      with_ignored(Nightrail.config.ignored_exceptions - [ "ActiveRecord::RecordNotFound" ]) { report_missing_widget }

      expect(nightrail_records(:exception).sole[:class]).to eq("ActiveRecord::RecordNotFound")
    end
    ...
    it "drops a subclass of an ignored exception, not just an exact class match" do
      ...
      with_ignored([ "ArgumentError" ]) do
        Nightrail.start_execution(source: :command, sample_kind: :commands)
        Nightrail.report(IgnoredSubclass.new("child of an ignored class"), handled: true)
        Nightrail.finish_execution
      end
    ...
    it "leaves an unlisted exception alone" do
      ...
      expect(nightrail_records(:exception).sole[:class]).to eq("RuntimeError")
    end
  end

  it "captures redacted local variables at the raise site when capture_exception_locals is on" do
    Nightrail.config.capture_exception_locals = true
    Nightrail::Subscribers::Exceptions::Locals.install!
    begin
      Nightrail.start_execution(source: :command, sample_kind: :commands)
      begin
        widget_id = 42
      ...
      locals = nightrail_records(:exception).sole[:locals]
      expect(locals["widget_id"]).to eq("42")
      expect(locals["password"]).to eq("[FILTERED]")
    ensure
      Nightrail::Subscribers::Exceptions::Locals.uninstall!
      Nightrail.config.capture_exception_locals = false
    end
  end
  ...
  it "falls back to the class when a local value's inspect raises" do
    ...
    value = Nightrail::Subscribers::Exceptions::Locals.inspect_value(explosive.new)
