...
RSpec.describe "request record", type: :request do
  ...
  it "groups requests by an MD5 hash of the HTTP method and route pattern" do
    ...
    req = nightrail_records(:request).sole
    expect(req[:_group]).to eq(Nightrail::Record.group_hash("GET", "/widgets(.:format)"))
  end
  ...
  it "strips every query parameter from the request URL without changing route grouping" do
    ...
    requests = nightrail_records(:request)
    ...
    expect(requests.map { |request| request[:_group] }.uniq).to eq([ Nightrail::Record.group_hash("GET", "/widgets(.:format)") ])
  end
  ...
  it "strips a fragment even when a URL has no query string" do
    expect(Nightrail::Record.url_without_sensitive_components("https://example.test/widgets#token=secret", limit: 2048))
  ...
  it "captures rate limit details once the native rate_limit threshold is exceeded" do
    ...
    reqs = nightrail_records(:request)
  ...
  describe "queue_time" do
    ...
    def queue_time_for(header, name: "X-Request-Start")
      ...
      nightrail_records(:request).sole[:queue_time]
    end

    it "is nil when no proxy stamped the request" do
      get "/widgets"

      expect(nightrail_records(:request).sole[:queue_time]).to be_nil
    end
