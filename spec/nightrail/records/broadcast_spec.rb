...
RSpec.describe "broadcast record" do
  ...
  it "captures kind broadcast, stream, byte size, and JSON coder for an ActionCable.server.broadcast" do
    ...
    rec = nightrail_records(:broadcast).sole
  ...
  it "groups two streams that differ only by a numeric id under the same shape" do
    ...
    groups = nightrail_records(:broadcast).map { |r| r[:_group] }.uniq
  ...
  context "channel transmit and perform_action", type: :channel do
    ...
    it "opens a production-shaped channel execution around the complete action" do
      ...
      parent = nightrail_records(:channel_action).sole
      children = nightrail_records.select { |r| r[:execution_id] == parent[:execution_id] && r[:t] != "channel_action" }
    ...
    it "captures kind transmit with the channel class and byte size when a channel calls #transmit" do
      ...
      transmit_rec = nightrail_records(:broadcast).find { |r| r[:kind] == "transmit" }
    ...
    it "captures kind perform_action with the channel class and action name" do
      ...
      action_rec = nightrail_records(:broadcast).find { |r| r[:kind] == "perform_action" }
    ...
    it "leaves no execution on the pooled worker when user resolution raises" do
      allow(Nightrail::Subscribers::Users).to receive(:resolve_from_current).and_raise("resolver blew up")
      subscribe

      perform :follow, "id" => 42

      expect(Nightrail.execution).to be_nil
      expect(nightrail_records(:channel_action).sole).to include(channel: "WidgetChannel", action: "follow")
    end

    it "honours the channel-specific sample rate" do
      Nightrail.config.sample[:channels] = 0.0
      subscribe
      perform :follow, "id" => 42

      expect(nightrail_records(:channel_action)).to be_empty
      expect(nightrail_records(:broadcast)).to be_empty
      expect(Nightrail.execution).to be_nil
    end
    ...
    it "still ships an unhandled exception and its channel parent when the action is sampled out" do
      ...
      expect(nightrail_records(:exception).sole[:execution_source]).to eq("channel_action")
      expect(nightrail_records(:channel_action).sole).to include(status: "failed", failed: true)
      expect(nightrail_records(:broadcast)).to be_empty
      expect(Nightrail.execution).to be_nil
    end
    ...
    it "reports an exception raised inside a channel action as an unhandled application.action_cable error" do
      ...
      expect(nightrail_records(:broadcast).find { |r| r[:kind] == "perform_action" }[:failed]).to be(true)
