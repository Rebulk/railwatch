...
RSpec.describe "browser exception record", type: :request do
  ...
  def post_beacon(payload)
    post "/nightrail/beacon", params: payload.to_json,
  ...
  def post_error(error)
    ...
    nightrail_records(:exception).sole
  end
  ...
  it "groups two occurrences whose messages differ only in their variable data together" do
    ...
    expect(nightrail_records(:exception).map { |ex| ex[:_group] }.uniq.size).to eq(1)
  end
  ...
  it "carries the page url, component, visit, session id, and user agent in the context" do
    ...
    context = JSON.parse(nightrail_records(:exception).sole[:context]).fetch("browser")
  ...
  it "caps at 50 errors from a single beacon POST even when more are sent" do
    ...
    expect(nightrail_records(:exception).size).to eq(50)
  end
  ...
  it "asks the app's beacon_user block who is behind the beacon, and describes them like any user" do
    ...
    Nightrail.config.beacon_user { |request| request.headers["X-Session"] == "tok" ? session_user : nil }

    post "/nightrail/beacon", params: { errors: [ { name: "TypeError", message: "boom", stack: STACK } ] }.to_json,
         headers: { "Content-Type" => "application/json", "X-Session" => "tok" }

    expect(nightrail_records(:exception).sole[:user]).to eq("7")
    expect(nightrail_records(:user).sole).to include(id: "7", name: "Ada", email: "ada@example.com")
  ensure
    Nightrail.config.beacon_user
  end

  it "falls back to the request-style lookup when the beacon_user block returns nil" do
    Nightrail.config.beacon_user { |_request| nil }
  ...
  it "records no user when the beacon_user block raises" do
    Nightrail.config.beacon_user { |_request| raise "session store down" }
  ...
  it "still records the beacon's visits alongside its errors" do
    ...
    expect(nightrail_records(:visit).size).to eq(1)
    expect(nightrail_records(:exception).size).to eq(1)
  end
  ...
  describe "a payload the app did not write" do
    it "drops entries that are not error objects instead of answering 500" do
      ...
      expect(response).to have_http_status(:no_content)
      expect(nightrail_records(:exception).sole[:class]).to eq("TypeError")
    end
    ...
    it "ignores a session that is not an object" do
      ...
      expect(response).to have_http_status(:no_content)
      expect(nightrail_records(:session)).to be_empty
      expect(nightrail_records(:exception).sole[:context]).not_to include("not an object")
    end
  ...
  describe "breadcrumbs" do
    def post_crumbs(crumbs)
      ...
      JSON.parse(nightrail_records(:exception).sole[:context]).fetch("browser")["breadcrumbs"]
    end
    ...
    it "leaves breadcrumbs out entirely when the client sent none" do
      ...
      expect(JSON.parse(nightrail_records(:exception).sole[:context]).fetch("browser")).not_to have_key("breadcrumbs")
    end
  end
  ...
  describe "context from reportError" do
    it "merges what the app passed alongside the browser's own fields" do
      ...
      context = JSON.parse(nightrail_records(:exception).sole[:context])
    ...
    it "flattens each value and keeps at most twenty keys, whatever the app sent" do
      ...
      context = JSON.parse(nightrail_records(:exception).sole[:context]).except("browser")
    ...
    it "ignores a context that is not an object" do
      ...
      expect(response).to have_http_status(:no_content)
      expect(JSON.parse(nightrail_records(:exception).sole[:context]).keys).to eq([ "browser" ])
    end
  end
  ...
  describe "the tenant hint" do
    it "stamps the tenant the client reported, since the beacon is outside the app's tenant scoping" do
      ...
      expect(nightrail_records(:exception).sole[:tenant]).to eq("acme")
      expect(nightrail_records(:session)).to be_empty
    end
    ...
    it "prefers the tenant the server resolved for itself over the client's hint" do
      ...
      expect(nightrail_records(:exception).sole[:tenant]).to eq("resolved")
    end
    ...
    it "ignores a hint that is not a string" do
      ...
      expect(response).to have_http_status(:no_content)
      expect(nightrail_records(:exception).sole[:tenant]).to be_nil
    end
