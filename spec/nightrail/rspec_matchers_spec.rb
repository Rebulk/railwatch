...
RSpec.describe "Nightrail RSpec matchers" do
  # Every matcher runs its block through Nightrail::SpecHelper#nightrail_capture,
  ...
  describe "have_nightrail_queries" do
    it "counts the queries a block runs when nothing else is executing" do
      expect { Widget.count }.to have_nightrail_queries(exactly: 1)
    end

    it "satisfies an at_most bound that the block stays under" do
      expect { Widget.count }.to have_nightrail_queries(at_most: 5)
    end

    it "satisfies an at_least bound that the block clears" do
      expect { 3.times { Widget.count } }.to have_nightrail_queries(at_least: 3)
    end

    it "counts zero for a block that touches no database" do
      expect { 1 + 1 }.to have_nightrail_queries(exactly: 0)
    end

    it "fails with the offending SQL when the block runs more queries than allowed" do
      message = failure_from(have_nightrail_queries(at_most: 1)) { 3.times { Widget.count } }
    ...
    it "truncates each listed statement to 120 characters so CI output stays readable" do
      ...
      message = failure_from(have_nightrail_queries(exactly: 0)) do
    ...
    it "reports the count in the negated failure message too" do
      message = negated_failure_from(have_nightrail_queries(at_most: 5)) { Widget.count }
    ...
    it "counts queries a request spec's own execution produced" do
      expect { Widget.count }.to have_nightrail_queries(at_least: 1)
    end

    it "rejects being given more than one bound" do
      expect { expect { Widget.count }.to have_nightrail_queries(at_most: 1, at_least: 1) }
        .to raise_error(ArgumentError, /exactly one of/)
    end

    it "rejects being given no bound at all" do
      expect { expect { Widget.count }.to have_nightrail_queries({}) }
        .to raise_error(ArgumentError, /exactly one of/)
    end

    it "describes itself in terms of the bound" do
      expect(have_nightrail_queries(at_most: 5).description).to eq("run at most 5 database queries")
    end
  end

  describe "have_nightrail_n_plus_one" do
    around do |example|
    ...
    it "passes when a block repeats one query shape past the threshold" do
      ...
      expect { 4.times { |i| Widget.where(id: i).to_a } }.to have_nightrail_n_plus_one
    end

    it "passes the negated form for a block with no repeated query shape" do
      expect { Widget.count }.not_to have_nightrail_n_plus_one
    end

    it "lists the repeated statement, count, and source in the negated failure message" do
      message = negated_failure_from(have_nightrail_n_plus_one) { 4.times { |i| Widget.where(id: i).to_a } }
    ...
    it "names the configured threshold in the positive failure message" do
      message = failure_from(have_nightrail_n_plus_one) { Widget.count }
  ...
  describe "record_nightrail_span" do
    it "passes when the block records a span with that name" do
      expect { record_span("checkout.total") }.to record_nightrail_span("checkout.total")
    end

    it "passes with no name for any span at all" do
      expect { record_span("anything") }.to record_nightrail_span(nil)
    end

    it "does not match a span recorded under a different name" do
      expect { record_span("checkout.tax") }.not_to record_nightrail_span("checkout.total")
    end

    it "lists the span names that were recorded when none matched" do
      message = failure_from(record_nightrail_span("checkout.total")) { record_span("checkout.tax") }
  ...
  describe "record_nightrail_exception" do
    it "passes when the block reports an exception of that class" do
      expect { Nightrail.report(ArgumentError.new("boom")) }.to record_nightrail_exception(ArgumentError)
    end

    it "does not match a different exception class" do
      expect { Nightrail.report(ArgumentError.new("boom")) }.not_to record_nightrail_exception(TypeError)
    end

    it "lists the exceptions that were recorded when the class did not match" do
      message = failure_from(record_nightrail_exception(TypeError)) { Nightrail.report(ArgumentError.new("boom")) }
  ...
  describe "record_nightrail_exceptions" do
    it "passes the negated form for a block that reports nothing" do
      expect { Widget.count }.not_to record_nightrail_exceptions
    end

    it "lists every recorded exception in the negated failure message" do
      message = negated_failure_from(record_nightrail_exceptions) do
        Nightrail.report(ArgumentError.new("first"))
        Nightrail.report(TypeError.new("second"))
      end
  ...
  describe "have_nightrail_outgoing_requests" do
    it "counts outgoing HTTP a block makes" do
      expect { Net::HTTP.get(URI("http://example.test/one")) }.to have_nightrail_outgoing_requests(exactly: 1)
    end

    it "counts zero for a block that makes none" do
      expect { Widget.count }.to have_nightrail_outgoing_requests(at_most: 0)
    end

    it "lists the method and url of each request when the bound is exceeded" do
      message = failure_from(have_nightrail_outgoing_requests(at_most: 1)) do
  ...
  describe "capture mechanics" do
    ...
    it "reads records off an already-open execution rather than waiting for it to finish" do
      Nightrail.start_execution(source: :request).sampled = true
      begin
        expect { Widget.count }.to have_nightrail_queries(exactly: 1)
        expect(Nightrail.execution.source).to eq(:request)
      ensure
    ...
    it "closes the execution it opened even when the block raises" do
      expect { expect { raise "nope" }.to have_nightrail_queries(exactly: 0) }.to raise_error("nope")

      expect(Nightrail.execution).to be_nil
    end

    it "raises rather than silently reporting an empty block when Nightrail is disabled" do
      old_token = Nightrail.config.token
      Nightrail.config.token = nil

      expect { expect { Widget.count }.to have_nightrail_queries(exactly: 0) }
        .to raise_error(Nightrail::SpecHelper::Disabled, /Set NIGHTRAIL_TOKEN/)
    ensure
      Nightrail.config.token = old_token
    end

    it "measures the block even when the app's request sample rate is zero" do
      Nightrail.config.sample = Nightrail.config.sample.merge(requests: 0.0)
...
RSpec.describe "Nightrail RSpec matchers in a request spec", type: :request do
  it "counts the queries a whole request ran" do
    Widget.create!(name: "w", gadget: Gadget.create!(name: "g"))

    expect { get "/widgets" }.to have_nightrail_queries(at_least: 1)
  end

  it "sees the outgoing HTTP a controller action made" do
    expect { get "/outbound" }.to have_nightrail_outgoing_requests(exactly: 1)
  end
end
