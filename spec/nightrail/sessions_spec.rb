...
RSpec.describe Nightrail::Sessions, type: :request do
  ...
  let(:cookie) { { "HTTP_COOKIE" => "other=1; nightrail_session=s1" } }
  ...
  def flushed
    ...
    nightrail_records(:session).last
  end
  ...
  it "accepts the session id from an X-Nightrail-Session header" do
    get "/widgets", headers: { "X-Nightrail-Session" => "hdr1" }

    expect(flushed).to include(id: "hdr1")
  end

  it "keys the session on the resolved user when the request carries no session id" do
    allow(Nightrail::Subscribers::Users).to receive(:resolve_id).and_return("7")
  ...
  it "ends and forgets a session idle for longer than config.session_timeout" do
    get "/widgets", headers: cookie
    Nightrail.config.session_timeout = 0.0

    expect(flushed).to include(id: "s1", ended: true)
    expect(sessions).to be_empty
  ensure
    Nightrail.config.session_timeout = 1800.0
  end
  ...
  it "keeps flushing a live session so a long one is one row per interval" do
    ...
    expect(nightrail_records(:session).map { |r| r[:id] }).to eq(%w[s1 s1])
    expect(sessions.keys).to eq(%w[s1])
  end

  it "caps the map at MAX_KEYS, dropping the oldest session and counting the drop" do
    stub_const("Nightrail::Sessions::MAX_KEYS", 2)
    exe = Nightrail::Execution.new(source: :request, sampled: true)

    %w[a b c].each { |id| described_class.touch(exe, { "HTTP_COOKIE" => "nightrail_session=#{id}" }, 200) }
  ...
  it "tracks nothing when track_sessions is off" do
    Nightrail.config.track_sessions = false
    ...
    Nightrail.config.track_sessions = true
  end

  it "still aggregates but ships nothing when sessions are ignored" do
    Nightrail.config.ignore = [ :sessions ]
  ...
  describe ".start!" do
    ...
    it "does not start a thread for a worker process, which never opens a session" do
      allow(Rails.env).to receive(:test?).and_return(false)
      allow(Nightrail::Subscribers::ProcessInfo).to receive(:role).and_return("worker")
    ...
    it "starts exactly one thread for a web process, and stop! flushes and shuts it down" do
      ...
      Nightrail.config.session_flush_interval = 30.0
      get "/widgets", headers: cookie
      ...
      expect(nightrail_records(:session).last).to include(id: "s1")
    ensure
      described_class.stop!
      Nightrail.config.session_flush_interval = 60.0
    end
  end
end

RSpec.describe "Nightrail::Sessions fork state" do
  it "replaces inherited synchronization, session, and drop state" do
    old_mutex = Nightrail::Sessions.instance_variable_get(:@mutex)
    old_wakeup = Nightrail::Sessions.instance_variable_get(:@wakeup)
    Nightrail::Sessions.instance_variable_set(:@sessions, "parent" => {})
    Nightrail::Sessions.instance_variable_set(:@dropped, 3)

    Nightrail::Sessions.restart_after_fork!

    expect(Nightrail::Sessions.instance_variable_get(:@mutex)).not_to equal(old_mutex)
    expect(Nightrail::Sessions.instance_variable_get(:@wakeup)).not_to equal(old_wakeup)
    expect(Nightrail::Sessions.instance_variable_get(:@sessions)).to be_empty
    expect(Nightrail::Sessions.dropped).to eq(0)
    expect(Nightrail::Sessions.instance_variable_get(:@pid)).to be_nil
  end
end
