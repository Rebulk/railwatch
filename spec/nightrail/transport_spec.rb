...
RSpec.describe Nightrail::Transport::Http do
  let(:transport) { described_class.new(Nightrail.config) }

  describe "#deliver" do
    it "pins TLS certificate verification to VERIFY_PEER" do
      config = Nightrail.config.dup
      config.ingest_url = "https://nightrail.example"
      secure_transport = described_class.new(config)
      response = Net::HTTPOK.new("1.1", "200", "OK")
      http = instance_double(Net::HTTP, request: response)

      expect(Net::HTTP).to receive(:start).with(
        "nightrail.example", 443,
    ...
    it "refuses non-loopback plain HTTP without making a request" do
      config = Nightrail.config.dup
      config.ingest_url = "http://nightrail.example"
    ...
    it "posts the batch as gzip NDJSON with the bearer token, gem version, and content headers" do
      ...
      expect(captured.headers["X-Nightrail-Version"]).to eq(Nightrail::VERSION)
      expect(captured.headers["X-Nightrail-Dropped"]).to eq("3")
      expect(captured.headers["X-Nightrail-Batch-Id"]).to match(/\A[0-9a-f-]{36}\z/)
    ...
    it "reports the active backpressure factor alongside batch accounting" do
      ...
      expect(captured.headers["X-Nightrail-Backpressure-Factor"]).to eq("4.0")
    end

    it "uses one caller-supplied batch id for the original request and its immediate retry" do
      seen_ids = []
      stub_request(:post, "http://nightrail.test/ingest").to_return do |request|
        seen_ids << request.headers["X-Nightrail-Batch-Id"]
    ...
    it "omits the X-Nightrail-Dropped header when nothing was dropped" do
      captured = nil
      stub_request(:post, "http://nightrail.test/ingest").to_return do |request|
        captured = request
        { status: 200, body: '{"accepted":1,"rejected":0}' }
      ...
      expect(captured.headers).not_to have_key("X-Nightrail-Dropped")
    end
    ...
    it "drops records that do not fit the batch byte cap rather than retrying forever" do
      ...
      expect(captured.headers["X-Nightrail-Dropped"]).to eq("1")
      decoded = Zlib::GzipReader.new(StringIO.new(captured.body)).read
    ...
    it "retries once on a network error, then returns a retryable result without raising" do
      seen_errors = []
      Nightrail.on_unrecoverable { |e| seen_errors << e }
      stub_request(:post, "http://nightrail.test/ingest").to_raise(Net::OpenTimeout)
      ...
      expect(a_request(:post, "http://nightrail.test/ingest")).to have_been_made.times(2)
      expect(seen_errors).to be_empty
    ensure
      Nightrail.config.on_unrecoverable = nil
    end
    ...
    it "retries a 5xx response once and succeeds on the retry" do
      stub_request(:post, "http://nightrail.test/ingest")
    ...
    it "retains a batch when a successful proxy response is HTML" do
      stub_request(:post, "http://nightrail.test/ingest").to_return(status: 200, body: "<html>sign in</html>")
    ...
    it "retains a batch when a successful response contains malformed JSON" do
      stub_request(:post, "http://nightrail.test/ingest").to_return(status: 200, body: '{"accepted":1')
    ...
    it "returns a valid partial acknowledgement with bounded rejection details" do
      ...
      stub_request(:post, "http://nightrail.test/ingest").to_return(
...
RSpec.describe Nightrail::Buffer do
  describe "#push" do
...
RSpec.describe Nightrail::Reporter do
  let(:reporter_config) do
    Nightrail.config.dup.tap do |config|
  ...
  def delivery_result(ok:, status: nil, error: nil, accepted: nil, rejected: nil, rejections: nil)
    Nightrail::Transport::Http::Result.new(ok: ok, status: status, error: error, accepted: accepted,
  ...
  def fork_pipe_transport(writer)
    Class.new do
      ...
      def deliver(records, dropped: 0)
        ...
        @io.flush
        Nightrail::Transport::Http::Result.new(ok: true, status: 200, accepted: records.size, rejected: 0)
      end
    end.new(writer)
  end
  ...
  describe "#flush" do
    it "passes the buffer's dropped count through to the transport" do
      ...
      small_config = Nightrail.config.dup
    ...
    it "passes its current backpressure factor through to supporting transports" do
      ...
      transport.define_singleton_method(:deliver) do |records, dropped: 0, backpressure_factor:|
        ...
        Nightrail::Transport::Http::Result.new(ok: true, status: 200, accepted: records.size)
      end
    ...
    it "splits a queue larger than one delivery instead of dropping the tail" do
      ...
      one = Nightrail::Record.buffered_bytes(record, limit: Float::INFINITY)
      deliveries = []
      transport = Object.new
      transport.define_singleton_method(:deliver) do |records, dropped: 0, batch_id:|
    ...
    it "keeps a failed request immutable while buffering records written during delivery" do
      ...
      transport.define_singleton_method(:deliver) do |records, dropped: 0, batch_id:|
        ...
        if attempts.one?
          ...
          Nightrail::Transport::Http::Result.new(ok: false, status: 408)
        else
          Nightrail::Transport::Http::Result.new(ok: true, status: 200, accepted: records.size)
        end
      end
    ...
    it "drops permanent client rejections and reports them explicitly" do
      seen_errors = []
      Nightrail.on_unrecoverable { |error| seen_errors << error }
      [ 401, 422 ].each do |status|
        transport = Object.new
        transport.define_singleton_method(:deliver) do |_records, dropped: 0|
          Nightrail::Transport::Http::Result.new(ok: false, status: status, error: "invalid payload")
        end
        reporter = described_class.new(reporter_config, transport: transport)
  ...
  describe "background delivery" do
    it "does not busy-loop while a retryable batch is backing off" do
      ...
      transport.define_singleton_method(:deliver) do |_records, dropped: 0|
        ...
        Nightrail::Transport::Http::Result.new(ok: false, status: 503)
      end
      config = reporter_config
  ...
  describe "shutdown" do
    ...
    it "retains and reports records it cannot send before the deadline" do
      ...
      transport = Object.new
      transport.define_singleton_method(:deliver) do |_records, dropped: 0|
        Nightrail::Transport::Http::Result.new(ok: false, status: 503, error: "unavailable")
      end
      config = reporter_config
  ...
  describe "forked processes" do
    it "keeps parent records and drop accounting out of the child, and emits child process and health records once", :aggregate_failures do
      ...
      pid = Process.fork do
        reader.close
        Nightrail.reporter.write({ t: "log", owner: "child" })
    ...
    it "replaces inherited locked reporter and buffer mutexes before the child records" do
      ...
      reporter = described_class.new(Nightrail.config, transport: Nightrail::SpecHelper::MemoryTransport.new)
      Nightrail.instance_variable_set(:@reporter, reporter)
    ...
    it "resets inherited retry, in-flight, and HTTP policy state without changing the parent" do
      ...
      reporter = described_class.new(Nightrail.config, transport: transport)
      ...
      pid = Process.fork do
        reader.close
        child_transport = Nightrail.reporter.instance_variable_get(:@transport)
        writer.puts(JSON.generate(
          unauthorized: child_transport.unauthorized?,
          flush_requested: Nightrail.reporter.instance_variable_get(:@flush_requested),
          urgent_at: Nightrail.reporter.instance_variable_get(:@urgent_at),
          retry_attempt: Nightrail.reporter.instance_variable_get(:@retry_attempt),
          retry_at: Nightrail.reporter.instance_variable_get(:@retry_at),
          retry_batch: Nightrail.reporter.instance_variable_get(:@retry_batch),
          backpressure_factor: Nightrail.reporter.backpressure_factor,
          in_flight_records: Nightrail.reporter.instance_variable_get(:@in_flight_records),
          in_flight_dropped: Nightrail.reporter.instance_variable_get(:@in_flight_dropped),
          shutdown_notified: Nightrail.reporter.instance_variable_get(:@shutdown_notified),
          shutdown_deadline: Nightrail.reporter.instance_variable_get(:@shutdown_deadline)
        ))
        writer.close
        exit!(0)
      end
  ...
  describe "shutdown on process exit" do
    it "flushes the buffer via the at_exit hook registered by the engine, even on SIGTERM", :aggregate_failures do
      ...
      pid = Process.fork do
        ...
        Nightrail.instance_variable_set(:@reporter, Nightrail::Reporter.new(Nightrail.config, transport: transport))
        Nightrail.reporter.buffer.push({ t: "log", message: "before term" })
        # No explicit trap: Nightrail relies on the at_exit hook the engine already
        # registered (lib/nightrail/engine.rb) at boot, inherited into this forked
...
RSpec.describe "fork handling" do
  it "registers one ActiveSupport::ForkTracker callback that resets the whole gem in the child" do
    ...
    ours = callbacks.select { |cb| cb.source_location&.first&.end_with?("lib/nightrail/engine.rb") }
    expect(ours.size).to eq(1)

    expect(Nightrail).to receive(:restart_after_fork!)
    ours.first.call
  end

  it "restarts the reporter before the health and session threads, so they emit into the child's reporter" do
    order = []
    allow(Nightrail::Profiler).to receive(:restart_after_fork!) { order << :profiler }
    allow(Nightrail.reporter).to receive(:restart_after_fork!) { order << :reporter }
    allow(Nightrail::Subscribers::Users).to receive(:restart_after_fork!) { order << :users }
    allow(Nightrail::Subscribers::ProcessInfo).to receive(:restart_after_fork!) { order << :process }
    allow(Nightrail::Health).to receive(:restart_after_fork!) { order << :health }
    allow(Nightrail::Sessions).to receive(:restart_after_fork!) { order << :sessions }
