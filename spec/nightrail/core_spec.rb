...
RSpec.describe Nightrail do
  describe Nightrail::SqlNormalizer do
    ...
    it "deep-freezes cached values so callers cannot corrupt later cache hits" do
      ...
      expect(cached[0]).to eq(Nightrail::Record.group_hash("mutation-test", cached[1]))
    end
  ...
  describe Nightrail::Redactor do
    it "masks configured headers and app filter_parameters" do
      r = described_class.new(Nightrail.config)
  ...
  describe Nightrail::Execution do
  ...
  describe "before_ingest" do
    it "drops a batch when a hook returns false" do
      Nightrail.before_ingest { |_batch| false }
      Nightrail.record(:log, level: "info", message: "x", tags: [], context: "{}")
      expect(nightrail_records).to be_empty
    ensure
      Nightrail.config.before_ingest.clear
    end
  end
  ...
  describe "transport" do
    it "posts gzip NDJSON with the bearer token and reports drops" do
      ...
      expect(WebMock).to have_requested(:post, "http://nightrail.test/ingest")
        .with(headers: { "Authorization" => "Bearer test-token", "Content-Encoding" => "gzip", "X-Nightrail-Dropped" => "3" })
    end

    it "never raises when the platform is down" do
      stub_request(:post, "http://nightrail.test/ingest").to_timeout
      result = Nightrail::Transport::Http.new(Nightrail.config).deliver([ { t: "log" } ])
  ...
  describe "commands" do
    it "records a rake task as a command execution" do
      require "rake"
      Rake::Task.define_task(:nightrail_demo) { Widget.count }
      Rake::Task[:nightrail_demo].execute
      cmd = nightrail_records(:command).sole
      expect(cmd).to include(name: "nightrail_demo", exit_code: 0)
  ...
  describe "process info" do
    it "reports boot_seconds measured from Nightrail::BOOTED_AT, not an unset global" do
      Nightrail::Subscribers::ProcessInfo.install!(Rails.application)
      Nightrail::Subscribers::ProcessInfo.record!
      proc_record = nightrail_records(:process).sole
  ...
  describe "bin/rails runner instrumentation" do
    it "records a command execution for a runner invocation" do
      fake = Class.new do
        prepend Nightrail::Patches::RunnerCommand
  ...
  describe "exception code and sql_state" do
    it "captures the errno for a SystemCallError" do
      Nightrail.report(Errno::ECONNREFUSED.new("refused"), handled: true)
      ex = nightrail_records(:exception).sole
  ...
  describe "transactions" do
    it "reports statement_count for the writes made inside a transaction" do
      require "rake"
      Rake::Task.define_task(:nightrail_txn_demo) do
      ...
      Rake::Task[:nightrail_txn_demo].execute
      txn = nightrail_records(:transaction).sole
  ...
  describe "self-monitoring" do
    it "reports a bad token once, not on every flush after it" do
      stub_request(:post, "http://nightrail.test/ingest").to_return(status: 401, body: "unauthorized")
      errors = []
      Nightrail.on_unrecoverable { |e| errors << e }
      reporter = Nightrail::Reporter.new(Nightrail.config)
      ...
      expect(a_request(:post, "http://nightrail.test/ingest")).to have_been_made.once
    ensure
      Nightrail.config.on_unrecoverable = nil
    end

    it "calls on_unrecoverable when ingest permanently rejects a batch" do
      stub_request(:post, "http://nightrail.test/ingest").to_return(status: 422, body: "boom")
      errors = []
      Nightrail.on_unrecoverable { |e| errors << e }
      reporter = Nightrail::Reporter.new(Nightrail.config)
      reporter.buffer.push({ t: "log" })
      ...
      expect(errors.first).to be_a(Nightrail::Reporter::DeliveryError)
  ...
  describe Nightrail::Faraday do
    it "records an outgoing_request for a Faraday connection using the middleware" do
      ...
      conn = ::Faraday.new("https://api.example.test") { |f| f.use Nightrail::Faraday }
      Rake::Task.define_task(:nightrail_faraday_demo) { conn.get("/things") }
      Rake::Task[:nightrail_faraday_demo].execute
    ...
    it "records the error and re-raises when the connection fails" do
      ...
      conn = ::Faraday.new("https://api.example.test") { |f| f.use Nightrail::Faraday }
      Rake::Task.define_task(:nightrail_faraday_error_demo) { conn.get("/things") }
      expect { Rake::Task[:nightrail_faraday_error_demo].execute }.to raise_error(Faraday::ConnectionFailed)
      out = nightrail_records(:outgoing_request).sole
      expect(out[:error]).to include("down")
    end
  end

  describe "Nightrail.instrument_outgoing" do
    it "records an outgoing_request and returns the block's value" do
      require "rake"
      result = nil
      Rake::Task.define_task(:nightrail_outgoing_demo) do
        result = Nightrail.instrument_outgoing(:get, "https://api.example.test/things") { Struct.new(:status).new(204) }
      end
      Rake::Task[:nightrail_outgoing_demo].execute
