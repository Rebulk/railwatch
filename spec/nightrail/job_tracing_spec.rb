...
RSpec.describe Nightrail::JobTracing, type: :request do
  it "round-trips nightrail_trace_id and nightrail_parent_id through serialize/deserialize" do
    job = WidgetJob.new("hello")
    job.nightrail_trace_id = "trace-abc"
    job.nightrail_parent_id = "parent-xyz"

    data = job.serialize
    expect(data["nightrail_trace_id"]).to eq("trace-abc")
    expect(data["nightrail_parent_id"]).to eq("parent-xyz")

    loaded = WidgetJob.new
    loaded.deserialize(data)
    expect(loaded.nightrail_trace_id).to eq("trace-abc")
    expect(loaded.nightrail_parent_id).to eq("parent-xyz")
  end

  it "round-trips nightrail_user and nightrail_tenant through serialize/deserialize" do
    job = WidgetJob.new("hello")
    job.nightrail_user = "acme:7"
    job.nightrail_tenant = "acme"

    data = job.serialize
    expect(data["nightrail_user"]).to eq("acme:7")
    expect(data["nightrail_tenant"]).to eq("acme")

    loaded = WidgetJob.new
    loaded.deserialize(data)
    expect(loaded.nightrail_user).to eq("acme:7")
    expect(loaded.nightrail_tenant).to eq("acme")
  end

  it "omits nightrail_user and nightrail_tenant entirely when there is no identity to carry" do
    data = WidgetJob.new("hello").serialize

    expect(data).not_to have_key("nightrail_user")
    expect(data).not_to have_key("nightrail_tenant")
  end
  ...
  it "falls back to the current execution's trace_id/id when not explicitly set" do
    ...
    expect(data["nightrail_trace_id"]).to eq(exe.trace_id)
    expect(data["nightrail_parent_id"]).to eq(exe.id)
  ensure
  ...
  describe "user and tenant propagation" do
    ...
    before { Nightrail::Subscribers::Users.instance_variable_set(:@seen, {}) }
    ...
    it "attributes a job attempt and its child records to the user and tenant of the request that enqueued it" do
      ...
      query = nightrail_records(:query).find { |q| q[:execution_id] == attempt[:attempt_id] }
    ...
    it "carries the same originating identity through a job that enqueues another job" do
      ...
      attempt = nightrail_records(:job_attempt).find { |a| a[:name] == "WidgetJob" }
      expect(attempt[:user]).to eq("acme:1")
      expect(attempt[:tenant]).to eq("acme")
    end
    ...
    it "qualifies a propagated user on the worker when the tenant bound after resolution" do
      ...
      exe = Nightrail.start_execution(source: :command, sample_kind: :commands)
      exe.user_id = Nightrail::Subscribers::Users.resolve_from_current
    ...
    it "falls back to local resolution for a payload enqueued before these keys existed" do
      ...
      queued.delete("nightrail_user")
      queued.delete("nightrail_tenant")
      Current.user = User.first
    ...
    it "resolves the user locally for an inline perform_now, which never serializes" do
      ...
      expect(nightrail_records(:job_attempt).sole[:user]).to eq("1")
    ensure
      Current.user = nil
    end
  end
