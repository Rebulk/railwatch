...
RSpec.describe Nightrail::Console do
  ...
  # exactly as `bin/rails console` has already done by the time Nightrail's
  # initializers run. Quiet mode is process-wide, so every example puts the
  # config back.
  after do
    Nightrail.config.enabled = true
    Nightrail.config.capture_console = false
  end
  ...
  it "leaves a server process alone" do
    ...
    expect(Nightrail.enabled?).to be(true)
  end
  ...
  context "in a console process" do
    ...
    it "goes quiet, and reports having done so only once" do
      ...
      expect(Nightrail.enabled?).to be(false)
      expect(described_class.silence!).to be(false)
    end
    ...
    it "opens no issue for an exception raised at the prompt" do
      ...
      Nightrail.report(NoMethodError.new("undefined method 'destroy!' for nil"), handled: false)
    ...
    it "sends no process record, because a console is not a server" do
      ...
      expect(nightrail_records(:process)).to be_empty
    end
    ...
    it "leaves no background thread running behind the prompt" do
      described_class.silence!
      Nightrail.record(:process, pid: Process.pid, role: "console")

      expect(Nightrail.reporter.instance_variable_get(:@thread)).to be_nil
    end

    it "captures everything as usual when capture_console is on" do
      Nightrail.config.capture_console = true
  ...
  it "is called from the engine's console railtie block" do
    blocks = Nightrail::Engine.console # the registered blocks, when called without one
