...
RSpec.describe Nightrail::ReleaseDetector do
  ...
  it "stops at the first environment hit" do
    ...
    expect(detect(env) { |found| source = found }).to eq("nightrail_deploy")
    expect(source).to eq("NIGHTRAIL_DEPLOY")
  end
  ...
  it "truncates a sha from every source but preserves other release names" do
    expect(detect("NIGHTRAIL_DEPLOY" => sha)).to eq(sha[0, 12])
    expect(detect("NIGHTRAIL_DEPLOY" => "release-2026-09-06")).to eq("release-2026-09-06")
  end
