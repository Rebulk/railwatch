...
RSpec.describe Nightrail::Execution do
  ...
  describe "#buffer" do
    ...
    it "bounds a tree by bytes as well as by record count" do
      ...
      bytes = Nightrail::Record.buffered_bytes(record, limit: Float::INFINITY)
      previous = Nightrail.config.execution_buffer_bytes
      Nightrail.config.execution_buffer_bytes = bytes + 10
      exe = new_execution
      ...
      Nightrail.config.execution_buffer_bytes = previous
    end
  end
  ...
  describe "#envelope" do
    ...
    it "flows user_id and tenant into every record built for the execution" do
      ...
      record = Nightrail::Record.build(:query, exe, sql: "select 1")
  ...
  describe "paused_depth nesting" do
    it "resumes recording only once the outermost Nightrail.ignore block returns" do
      exe = new_execution

      Nightrail::Current.with(exe) do
  ...
  describe "#capture_memory" do
    ...
    before { Nightrail::Execution.instance_variable_set(:@memory_sampled_at, 0.0) }
    ...
    it "keeps the last good sample when /proc/self/statm becomes unreadable" do
      exe = new_execution
      Nightrail::Execution.instance_variable_set(:@memory_sample, nil)
