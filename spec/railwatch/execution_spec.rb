...
RSpec.describe Railwatch::Execution do
  ...
  describe "#buffer" do
    ...
    it "bounds a tree by bytes as well as by record count" do
      ...
      Railwatch.config.execution_buffer_bytes = bytes + 10
      exe = new_execution
  ...
  describe "#envelope" do
    ...
    it "flows user_id and tenant into every record built for the execution" do
      ...
      record = Railwatch::Record.build(:query, exe, sql: "select 1")
  ...
  describe "paused_depth nesting" do
    it "resumes recording only once the outermost Railwatch.ignore block returns" do
      exe = new_execution

      Railwatch::Current.with(exe) do
        expect(exe.recording?).to be(true)
  ...
  describe "#capture_memory" do
    ...
    before { Railwatch::Execution.instance_variable_set(:@memory_sampled_at, 0.0) }
    ...
    it "keeps the last good sample when /proc/self/statm becomes unreadable" do
      exe = new_execution
      Railwatch::Execution.instance_variable_set(:@memory_sample, nil)
