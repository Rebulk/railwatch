...
RSpec.describe Railwatch::Configuration do
  ...
  describe "env var defaults and overrides" do
    {
      "RAILWATCH_ENABLED" => [ :enabled, "0", false, true ],
      "RAILWATCH_TOKEN" => [ :token, "abc123", "abc123", nil ],
      "RAILWATCH_INGEST_URL" => [ :ingest_url, "https://custom.example", "https://custom.example", "https://railwatch.rebulk.com" ],
      "RAILWATCH_ALLOW_HTTP" => [ :allow_http, "1", true, false ],
      "RAILWATCH_SERVER" => [ :server, "web-1", "web-1", ENV["KAMAL_HOST"] || Socket.gethostname ],
      "RAILWATCH_LOG_LEVEL" => [ :log_level, "debug", :debug, :info ],
      "RAILWATCH_CAPTURE_REQUEST_PAYLOAD" => [ :capture_request_payload, "1", true, false ],
      "RAILWATCH_CAPTURE_EXCEPTION_SOURCE_CODE" => [ :capture_exception_source, "0", false, true ],
      "RAILWATCH_CAPTURE_JOB_RETRY_ERRORS" => [ :capture_job_retry_errors, "1", true, false ],
      "RAILWATCH_BUFFER_SIZE" => [ :buffer_size, "9999", 9_999, 10_000 ],
      ...
      "RAILWATCH_FLUSH_THRESHOLD" => [ :flush_threshold, "50", 50, 500 ],
      "RAILWATCH_CONNECT_TIMEOUT" => [ :connect_timeout, "0.5", 0.5, 1.0 ],
      "RAILWATCH_TIMEOUT" => [ :timeout, "9.0", 9.0, 3.0 ],
      "RAILWATCH_SHUTDOWN_TIMEOUT" => [ :shutdown_timeout, "4.0", 4.0, 2.0 ],
      "RAILWATCH_SLOW_QUERY_MS" => [ :slow_query_threshold_ms, "12.0", 12.0, 5.0 ],
      "RAILWATCH_N_PLUS_ONE_THRESHOLD" => [ :n_plus_one_threshold, "9", 9, 5 ],
      "RAILWATCH_FAILURE_CONTEXT" => [ :failure_context, "200", 200, 0 ],
      "RAILWATCH_HEALTH_INTERVAL" => [ :health_interval, "60.0", 60.0, 15.0 ],
      "RAILWATCH_CAPTURE_SQL_VALUES" => [ :capture_sql_values, "1", true, false ],
      "RAILWATCH_CAPTURE_QUERY_EXPLAIN" => [ :capture_query_explain, "1", true, false ],
      "RAILWATCH_EXPLAIN_THRESHOLD_MS" => [ :explain_threshold_ms, "250.0", 250.0, 100.0 ],
      "RAILWATCH_CAPTURE_RESCUED_EXCEPTIONS" => [ :capture_rescued_exceptions, "0", false, true ],
      "RAILWATCH_BEACON" => [ :beacon_enabled, "0", false, true ],
      "RAILWATCH_CAPTURE_CONSOLE" => [ :capture_console, "1", true, false ],
      "RAILWATCH_DETECT_DEPLOY" => [ :detect_deploy, "0", false, true ],
      "RAILWATCH_DEBUG" => [ :debug, "1", true, false ]
    ...
    it "keeps the documented default when a numeric variable is not a number, instead of silently zeroing it" do
      with_env("RAILWATCH_BUFFER_SIZE" => "12px", "RAILWATCH_FLUSH_INTERVAL" => "never",
               "RAILWATCH_BACKPRESSURE_HIGH_WATER" => "most",
               "RAILWATCH_REQUEST_SAMPLE_RATE" => "half") do |config|
    ...
    it "keeps the default when the backpressure high-water fraction is outside 0.0..1.0" do
      with_env("RAILWATCH_BACKPRESSURE_HIGH_WATER" => "2") do |config|
        expect(config.backpressure_high_water).to eq(0.8)
      end
      with_env("RAILWATCH_BACKPRESSURE_HIGH_WATER" => "-1") do |config|
        expect(config.backpressure_high_water).to eq(0.8)
      end
      with_env("RAILWATCH_BACKPRESSURE_HIGH_WATER" => "NaN") do |config|
        expect(config.backpressure_high_water).to eq(0.8)
      end
    end
    ...
    it "maps every execution sample-rate env var into config.sample, defaulting each kind to 1.0" do
      with_env(
        "RAILWATCH_REQUEST_SAMPLE_RATE" => "0.2", "RAILWATCH_JOB_SAMPLE_RATE" => "0.3",
        "RAILWATCH_COMMAND_SAMPLE_RATE" => "0.4", "RAILWATCH_SCHEDULED_TASK_SAMPLE_RATE" => "0.5",
        "RAILWATCH_CHANNEL_SAMPLE_RATE" => "0.6", "RAILWATCH_EXCEPTION_SAMPLE_RATE" => "0.7"
      ) do |config|
        expect(config.sample).to eq(requests: 0.2, jobs: 0.3, commands: 0.4, scheduled_tasks: 0.5,
                                    channels: 0.6, exceptions: 0.7)
      end
      with_env(
        "RAILWATCH_REQUEST_SAMPLE_RATE" => nil, "RAILWATCH_JOB_SAMPLE_RATE" => nil, "RAILWATCH_COMMAND_SAMPLE_RATE" => nil,
        "RAILWATCH_SCHEDULED_TASK_SAMPLE_RATE" => nil, "RAILWATCH_CHANNEL_SAMPLE_RATE" => nil,
        "RAILWATCH_EXCEPTION_SAMPLE_RATE" => nil
      ) do |config|
    ...
    it "maps RAILWATCH_DEPLOY, falling back to KAMAL_VERSION then GIT_REV, then whatever the platform or checkout exposes" do
      with_env("RAILWATCH_DEPLOY" => "v1", "KAMAL_VERSION" => "kamal-v", "GIT_REV" => "git-v") { |c| expect(c.deploy).to eq("v1") }
      with_env("RAILWATCH_DEPLOY" => nil, "KAMAL_VERSION" => "kamal-v", "GIT_REV" => "git-v") { |c| expect(c.deploy).to eq("kamal-v") }
      with_env("RAILWATCH_DEPLOY" => nil, "KAMAL_VERSION" => nil, "GIT_REV" => "git-v") { |c| expect(c.deploy).to eq("git-v") }
      ...
      cleared = Railwatch::ReleaseDetector::ENV_KEYS.to_h { |key| [ key, nil ] }
    ...
    it "uses only explicit and Kamal releases when deploy detection is disabled" do
      with_env("RAILWATCH_DETECT_DEPLOY" => "0", "RAILWATCH_DEPLOY" => nil,
    ...
    it "keeps an initializer deploy override when detection is toggled" do
      ...
      expect(config.deploy_source).to eq("config/initializers/railwatch.rb")
    end

    it "splits RAILWATCH_REDACT_HEADERS on commas, defaulting to the documented header list" do
      with_env("RAILWATCH_REDACT_HEADERS" => "X-Api-Key, X-Secret") do |config|
        expect(config.redact_headers).to eq(%w[X-Api-Key X-Secret])
      end
      with_env("RAILWATCH_REDACT_HEADERS" => nil) do |config|
        expect(config.redact_headers).to eq(%w[Authorization Cookie Set-Cookie Proxy-Authorization X-CSRF-Token X-XSRF-TOKEN])
      end
    end

    it "splits RAILWATCH_REDACT_PARAMS on commas, defaulting to the documented param list" do
      with_env("RAILWATCH_REDACT_PARAMS" => "ssn, credit_card") do |config|
        expect(config.redact_params).to eq(%w[ssn credit_card])
      end
      with_env("RAILWATCH_REDACT_PARAMS" => nil) do |config|
    ...
    described_class::RECORD_TYPES.each do |type|
      it "maps RAILWATCH_IGNORE_#{type.to_s.upcase} to ignoring #{type}, defaulting to not ignored" do
        with_env("RAILWATCH_IGNORE_#{type.to_s.upcase}" => "1") { |c| expect(c.ignored?(type)).to be(true) }
        with_env("RAILWATCH_IGNORE_#{type.to_s.upcase}" => nil) { |c| expect(c.ignored?(type)).to be(false) }
      end
    end

    it "treats True/YES/On as truthy for RAILWATCH_IGNORE_* regardless of case" do
      %w[1 true True TRUE yes Yes on ON].each do |value|
        with_env("RAILWATCH_IGNORE_QUERIES" => value) { |c| expect(c.ignored?(:queries)).to be(true) }
      end
  ...
  describe "#ingest_url_allowed?" do
    it "allows HTTPS and loopback HTTP, but refuses other HTTP by default" do
      with_env("RAILWATCH_ALLOW_HTTP" => nil, "RAILWATCH_INGEST_URL" => "https://railwatch.example") do |config|
        expect(config.ingest_url_allowed?).to be(true)
      end
      %w[localhost 127.0.0.1 ::1].each do |host|
        with_env("RAILWATCH_ALLOW_HTTP" => nil, "RAILWATCH_INGEST_URL" => "http://[#{host}]") do |config|
          config.ingest_url = "http://#{host}" unless host == "::1"
          expect(config.ingest_url_allowed?).to be(true)
        end
      end
      with_env("RAILWATCH_ALLOW_HTTP" => nil, "RAILWATCH_INGEST_URL" => "http://railwatch.example") do |config|
        expect(config.ingest_url_allowed?).to be(false)
      end
    end

    it "allows non-loopback HTTP only with the explicit escape hatch" do
      with_env("RAILWATCH_ALLOW_HTTP" => "true", "RAILWATCH_INGEST_URL" => "http://railwatch.example") do |config|
        expect(config.ingest_url_allowed?).to be(true)
      end
    end
  end
  ...
  describe "ignored_exceptions" do
    it "defaults to the Rails-relevant subset of Sentry's own excluded_exceptions" do
      with_env("RAILWATCH_IGNORED_EXCEPTIONS" => nil) do |config|
    ...
    it "splits RAILWATCH_IGNORED_EXCEPTIONS on commas, replacing the default list" do
      with_env("RAILWATCH_IGNORED_EXCEPTIONS" => "Foo::Bar, Baz") do |config|
  ...
  describe "ignored_request_paths" do
    it "defaults to health and beacon paths as a mutable copy" do
      with_env("RAILWATCH_IGNORED_REQUEST_PATHS" => nil) do |config|
        expect(config.ignored_request_paths).to eq(%w[/up /railwatch/beacon])
    ...
    it "splits RAILWATCH_IGNORED_REQUEST_PATHS on commas, replacing the defaults" do
      with_env("RAILWATCH_IGNORED_REQUEST_PATHS" => "/healthz, /ready, ") do |config|
  ...
  describe "interactive_runner_paths" do
    it "defaults to the two scratch roots, as a mutable copy" do
      with_env("RAILWATCH_INTERACTIVE_RUNNER_PATHS" => nil) do |config|
    ...
    it "splits RAILWATCH_INTERACTIVE_RUNNER_PATHS on commas, replacing the default list" do
      with_env("RAILWATCH_INTERACTIVE_RUNNER_PATHS" => "/scratch/, /var/tmp/") do |config|
  ...
  describe "#enabled?" do
    it "is false without a token even when RAILWATCH_ENABLED=1" do
      with_env("RAILWATCH_ENABLED" => "1", "RAILWATCH_TOKEN" => nil) { |c| expect(c.enabled?).to be(false) }
      with_env("RAILWATCH_ENABLED" => "1", "RAILWATCH_TOKEN" => "tok") { |c| expect(c.enabled?).to be(true) }
    end

    it "is false when RAILWATCH_ENABLED=0 even with a token" do
      with_env("RAILWATCH_ENABLED" => "0", "RAILWATCH_TOKEN" => "tok") { |c| expect(c.enabled?).to be(false) }
    end
  end
  ...
  describe "vendor default exclusion toggles" do
    ...
    it "maps RAILWATCH_CAPTURE_DEFAULT_VENDOR_COMMANDS/CACHE_KEYS to the toggles" do
      with_env("RAILWATCH_CAPTURE_DEFAULT_VENDOR_COMMANDS" => "1", "RAILWATCH_CAPTURE_DEFAULT_VENDOR_CACHE_KEYS" => "1") do |c|
...
RSpec.describe Railwatch::Configuration, "server under Kamal" do
  def with_env(pairs)
  ...
  it "stamps records with the Kamal host rather than the per-deploy container hostname" do
    with_env("KAMAL_HOST" => "5.78.218.245", "RAILWATCH_SERVER" => nil) do
      expect(described_class.new.server).to eq("5.78.218.245")
    end
  end

  it "still lets RAILWATCH_SERVER override the Kamal host" do
    with_env("KAMAL_HOST" => "5.78.218.245", "RAILWATCH_SERVER" => "web-1") do
