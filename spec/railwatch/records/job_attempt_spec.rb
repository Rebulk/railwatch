...
RSpec.describe "job_attempt record" do
  ...
  describe "arguments (capture_job_arguments)" do
    around do |example|
      Railwatch.config.capture_job_arguments = true
      example.run
    ensure
      Railwatch.config.capture_job_arguments = false
    end
    ...
    it "ships an Active Record argument as a GlobalID, never a hydrated model" do
      ...
      expect(railwatch_records(:job_attempt).sole[:arguments])
    ...
    it "redacts a password key inside a hash argument" do
      ...
      expect(railwatch_records(:job_attempt).sole[:arguments].sole)
    ...
    it "redacts hashes nested inside an array argument" do
      ...
      expect(railwatch_records(:job_attempt).sole[:arguments].sole.first)
