...
RSpec.describe "query record", type: :request do
  ...
  after { Railwatch.config.capture_sql_values = false }
  ...
  it "does not capture SQL literal values by default" do
    ...
    q = railwatch_records(:query).find { |r| r[:sql].start_with?("SELECT") }
  ...
  it "captures raw SQL only after an explicit opt-in" do
    Railwatch.config.capture_sql_values = true
    sql = "SELECT 'private-customer@example.test' AS secret"
    Railwatch.start_execution(source: :command, sample_kind: :commands)
    ActiveRecord::Base.connection.select_all(sql)
    Railwatch.finish_execution(:command, group: "g", class: "Rake::Task", name: "demo", command: "rake demo", exit_code: 0)
  ...
  it "bounds and repairs encoding in explicitly captured raw SQL" do
    ...
    q = railwatch_records(:query).find { |r| r[:name] == "Raw SQL" }
    expect(q[:sql]).to be_valid_encoding
    expect(q[:sql].length).to eq(Railwatch::Subscribers::Queries::MAX_SQL)
  end
  ...
  it "never ships Active Record's structured bind payload" do
    ...
    q = railwatch_records(:query).find { |r| r[:name] == "Widget Load" }
  ...
  it "captures sql, name, duration, connection, adapter, row_count, and in_transaction for a SELECT" do
    get "/widgets"
    q = railwatch_records(:query).find { |r| r[:sql].include?("FROM \"widgets\"") }
  ...
  it "captures affected_rows and in_transaction for a write made inside a transaction" do
    ...
    insert = railwatch_records(:query).find { |r| r[:sql].start_with?("INSERT") }
  ...
  it "skips SCHEMA and TRANSACTION named statements entirely" do
    get "/widgets"
    names = railwatch_records(:query).map { |r| r[:name] }
  ...
  describe "explain capture" do
    ...
    before do
      Railwatch::Subscribers::Queries.instance_variable_get(:@explained).clear
      Railwatch.config.capture_query_explain = true
      Railwatch.config.capture_sql_values = true
      Railwatch.config.explain_threshold_ms = 0.0
    end

    after do
      Railwatch.config.capture_query_explain = false
      Railwatch.config.capture_sql_values = false
      Railwatch.config.explain_threshold_ms = 100.0
    end
    ...
    it "never records the EXPLAIN statement itself as a query" do
      ...
      expect(railwatch_records(:query).map { |r| r[:sql] }).to all(satisfy { |sql| !sql.include?("EXPLAIN") })
    end
    ...
    it "explains a shape at most once per process within the throttle window" do
      ...
      widget_loads = railwatch_records(:query).select { |r| r[:sql].include?("FROM \"widgets\"") }
    ...
    it "leaves explain nil for a query faster than explain_threshold_ms" do
      Railwatch.config.explain_threshold_ms = 10_000.0
      get "/widgets"

      expect(railwatch_records(:query).map { |r| r[:explain] }.compact).to be_empty
    end
    ...
    it "still captures plans while sql itself stays normalized" do
      ...
      explained = railwatch_records(:query).reject { |r| r[:explain].nil? }
  ...
  it "computes a caller source location for a query shape" do
    get "/many"
    sources = railwatch_records(:query).select { |r| r[:sql].include?("FROM \"gadgets\"") }.map { |r| r[:source] }
