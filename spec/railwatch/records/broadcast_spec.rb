...
RSpec.describe "broadcast record" do
  ...
  it "captures kind broadcast, stream, byte size, and JSON coder for an ActionCable.server.broadcast" do
    ...
    rec = railwatch_records(:broadcast).sole
  ...
  it "groups two streams that differ only by a numeric id under the same shape" do
    ...
    groups = railwatch_records(:broadcast).map { |r| r[:_group] }.uniq
  ...
  context "channel transmit and perform_action", type: :channel do
    ...
    it "opens a production-shaped channel execution around the complete action" do
      ...
      parent = railwatch_records(:channel_action).sole
      children = railwatch_records.select { |r| r[:execution_id] == parent[:execution_id] && r[:t] != "channel_action" }
    ...
    it "captures kind transmit with the channel class and byte size when a channel calls #transmit" do
      ...
      transmit_rec = railwatch_records(:broadcast).find { |r| r[:kind] == "transmit" }
    ...
    it "captures kind perform_action with the channel class and action name" do
      ...
      action_rec = railwatch_records(:broadcast).find { |r| r[:kind] == "perform_action" }
    ...
    it "leaves no execution on the pooled worker when user resolution raises" do
      allow(Railwatch::Subscribers::Users).to receive(:resolve_from_current).and_raise("resolver blew up")
      subscribe

      perform :follow, "id" => 42

      expect(Railwatch.execution).to be_nil
      expect(railwatch_records(:channel_action).sole).to include(channel: "WidgetChannel", action: "follow")
    end

    it "honours the channel-specific sample rate" do
      Railwatch.config.sample[:channels] = 0.0
      subscribe
      perform :follow, "id" => 42

      expect(railwatch_records(:channel_action)).to be_empty
      expect(railwatch_records(:broadcast)).to be_empty
      expect(Railwatch.execution).to be_nil
    end
    ...
    it "still ships an unhandled exception and its channel parent when the action is sampled out" do
      ...
      expect(railwatch_records(:exception).sole[:execution_source]).to eq("channel_action")
      expect(railwatch_records(:channel_action).sole).to include(status: "failed", failed: true)
      expect(railwatch_records(:broadcast)).to be_empty
      expect(Railwatch.execution).to be_nil
    end
    ...
    it "reports an exception raised inside a channel action as an unhandled application.action_cable error" do
      ...
      expect(railwatch_records(:broadcast).find { |r| r[:kind] == "perform_action" }[:failed]).to be(true)
