...
RSpec.describe "health record" do
  ...
  # surface Railwatch::Health actually reads.
  ...
  it "captures pid, role, memory, Puma, connection pool, and Solid Queue stats in one record" do
    ...
    health = railwatch_records(:health).sole
  ...
  it "reports queue_latency as the age of the oldest ready job, in microseconds" do
    ...
    expected = (Railwatch::Clock.now - oldest_ready_at.to_f) * 1_000_000
    expect(railwatch_records(:health).sole[:queue_latency]).to be_within(60_000_000).of(expected)
  end
  ...
  it "packs queues, workers, requests_count, running, and max_threads_reached into detail" do
    ...
    detail = JSON.parse(railwatch_records(:health).sole[:detail])
  ...
  it "packs the recurring task schedule into detail, so the platform knows which tasks are still configured" do
    allow(Railwatch::Subscribers::Jobs).to receive(:recurring_tasks).and_return(
      keys: %w[nightly_cleanup], classes: %w[CleanupJob], schedules: { "nightly_cleanup" => "0 2 * * *" })

    Railwatch::Health.sample

    expect(JSON.parse(railwatch_records(:health).sole[:detail])["recurring_tasks"]).to eq("nightly_cleanup" => "0 2 * * *")
  end

  it "leaves recurring_tasks out of detail entirely when none are configured or the table could not be read" do
    allow(Railwatch::Subscribers::Jobs).to receive(:recurring_tasks).and_return(keys: [], classes: [], schedules: {})

    Railwatch::Health.sample

    expect(JSON.parse(railwatch_records(:health).sole[:detail])).not_to have_key("recurring_tasks")
  end
  ...
  it "never raises out of a sample, even when writing the record fails" do
    ...
    expect(Railwatch::Health.sample).to be_nil
  end
end

RSpec.describe Railwatch::Health do
  describe ".start!" do
    ...
    it "does not start a thread for a console or command process" do
      ...
      allow(Railwatch::Subscribers::ProcessInfo).to receive(:role).and_return("command")
    ...
    it "starts exactly one thread for a web process, and stop! shuts it down" do
      ...
      Railwatch.config.health_interval = 30.0
      ...
      described_class.stop!
      Railwatch.config.health_interval = 15.0
    end
  end
end

RSpec.describe "Railwatch::Health fork state" do
  it "replaces synchronization state inherited by the child" do
    old_mutex = Railwatch::Health.instance_variable_get(:@mutex)
    old_wakeup = Railwatch::Health.instance_variable_get(:@wakeup)

    Railwatch::Health.restart_after_fork!
