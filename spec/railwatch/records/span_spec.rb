...
RSpec.describe "span record" do
  ...
  it "ships as a v1 span with the name, its group hash, and no attributes when none were given" do
    ...
    expect(span[:_group]).to eq(Railwatch::Record.group_hash("pdf.render"))
  ...
  it "belongs to the execution it ran inside" do
    Railwatch.start_execution(source: :command, sample_kind: :commands)
    Railwatch.span("pdf.render") { :done }
    Railwatch.finish_execution(:command, group: "g", class: "Rake::Task", name: "demo",
                             command: "rake demo", exit_code: 0)
