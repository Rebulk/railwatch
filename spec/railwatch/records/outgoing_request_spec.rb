...
RSpec.describe "outgoing_request record" do
  ...
  it "does not ship a record for a request to the ingest host itself" do
    stub_request(:get, "http://railwatch.test/ingest").to_return(status: 200, body: "ok")

    Railwatch.start_execution(source: :command, sample_kind: :commands)
    Net::HTTP.get(URI("http://railwatch.test/ingest"))
    finish!
  ...
  it "records exactly once for a single request made via Net::HTTP.start, not doubled by the reentry guard" do
    ...
    expect(railwatch_records(:outgoing_request).size).to eq(1)
  end
  ...
  it "instrument_outgoing records an outgoing_request for a non-Net::HTTP client result that responds to #status" do
    ...
    Railwatch.start_execution(source: :command, sample_kind: :commands)
    result = Railwatch.instrument_outgoing("GET", "http://custom-adapter.test/x") { fake_response }
    finish!
  ...
  it "instrument_outgoing does not record anything when the yielded result has no #status" do
    Railwatch.start_execution(source: :command, sample_kind: :commands)
    result = Railwatch.instrument_outgoing("GET", "http://custom-adapter.test/x") { "plain string" }
    finish!
  ...
  describe "response_body (capture_response_body_on_error)" do
    ...
    context "when the option is on" do
      around do |example|
        Railwatch.config.capture_response_body_on_error = true
        example.run
      ensure
        Railwatch.config.capture_response_body_on_error = false
      end
