...
RSpec.describe Railwatch do
  describe Railwatch::SqlNormalizer do
    ...
    it "deep-freezes cached values so callers cannot corrupt later cache hits" do
      ...
      expect(cached[0]).to eq(Railwatch::Record.group_hash("mutation-test", cached[1]))
    end
  ...
  describe Railwatch::Redactor do
    it "masks configured headers and app filter_parameters" do
      r = described_class.new(Railwatch.config)
  ...
  describe Railwatch::Execution do
  ...
  describe "before_ingest" do
    it "drops a batch when a hook returns false" do
      Railwatch.before_ingest { |_batch| false }
      Railwatch.record(:log, level: "info", message: "x", tags: [], context: "{}")
      expect(railwatch_records).to be_empty
    ensure
      Railwatch.config.before_ingest.clear
    end
  end
  ...
  describe "transport" do
    it "posts gzip NDJSON with the bearer token and reports drops" do
      ...
      expect(WebMock).to have_requested(:post, "http://railwatch.test/ingest")
        .with(headers: { "Authorization" => "Bearer test-token", "Content-Encoding" => "gzip", "X-Railwatch-Dropped" => "3" })
    end

    it "never raises when the platform is down" do
      stub_request(:post, "http://railwatch.test/ingest").to_timeout
      result = Railwatch::Transport::Http.new(Railwatch.config).deliver([ { t: "log" } ])
  ...
  describe "commands" do
    it "records a rake task as a command execution" do
      require "rake"
      Rake::Task.define_task(:railwatch_demo) { Widget.count }
      Rake::Task[:railwatch_demo].execute
  ...
  describe "process info" do
    it "reports boot_seconds measured from Railwatch::BOOTED_AT, not an unset global" do
      Railwatch::Subscribers::ProcessInfo.install!(Rails.application)
      Railwatch::Subscribers::ProcessInfo.record!
      proc_record = railwatch_records(:process).sole
  ...
  describe "bin/rails runner instrumentation" do
    it "records a command execution for a runner invocation" do
      fake = Class.new do
        prepend Railwatch::Patches::RunnerCommand
  ...
  describe "exception code and sql_state" do
    it "captures the errno for a SystemCallError" do
      Railwatch.report(Errno::ECONNREFUSED.new("refused"), handled: true)
      ex = railwatch_records(:exception).sole
  ...
  describe "transactions" do
    it "reports statement_count for the writes made inside a transaction" do
      require "rake"
      Rake::Task.define_task(:railwatch_txn_demo) do
      ...
      Rake::Task[:railwatch_txn_demo].execute
  ...
  describe "default vendor command exclusion" do
    it "emits no command record for a default vendor rake task unless opted in" do
      ...
      Railwatch.config.capture_default_vendor_commands = true
      Rake::Task[:"db:migrate"].execute
      expect(railwatch_records(:command).sole[:name]).to eq("db:migrate")
    ensure
      Railwatch.config.capture_default_vendor_commands = false
    end
  end

  describe "self-monitoring" do
    it "reports a bad token once, not on every flush after it" do
      stub_request(:post, "http://railwatch.test/ingest").to_return(status: 401, body: "unauthorized")
      errors = []
      Railwatch.on_unrecoverable { |e| errors << e }
    ...
    it "calls on_unrecoverable when ingest permanently rejects a batch" do
      stub_request(:post, "http://railwatch.test/ingest").to_return(status: 422, body: "boom")
      errors = []
      Railwatch.on_unrecoverable { |e| errors << e }
      reporter = Railwatch::Reporter.new(Railwatch.config)
      reporter.buffer.push({ t: "log" })
      ...
      expect(errors.first).to be_a(Railwatch::Reporter::DeliveryError)
  ...
  describe Railwatch::Faraday do
    it "records an outgoing_request for a Faraday connection using the middleware" do
      ...
      conn = ::Faraday.new("https://api.example.test") { |f| f.use Railwatch::Faraday }
      Rake::Task.define_task(:railwatch_faraday_demo) { conn.get("/things") }
      Rake::Task[:railwatch_faraday_demo].execute
    ...
    it "records the error and re-raises when the connection fails" do
      ...
      conn = ::Faraday.new("https://api.example.test") { |f| f.use Railwatch::Faraday }
      Rake::Task.define_task(:railwatch_faraday_error_demo) { conn.get("/things") }
      expect { Rake::Task[:railwatch_faraday_error_demo].execute }.to raise_error(Faraday::ConnectionFailed)
      out = railwatch_records(:outgoing_request).sole
      expect(out[:error]).to include("down")
    end
  end

  describe "Railwatch.instrument_outgoing" do
    it "records an outgoing_request and returns the block's value" do
      require "rake"
      result = nil
      Rake::Task.define_task(:railwatch_outgoing_demo) do
        result = Railwatch.instrument_outgoing(:get, "https://api.example.test/things") { Struct.new(:status).new(204) }
      end
      Rake::Task[:railwatch_outgoing_demo].execute
