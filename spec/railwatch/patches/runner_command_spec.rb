...
RSpec.describe Railwatch::Patches::RunnerCommand do
  ...
  describe "a script in a scratch directory" do
    it "ships an interactive command record and opens no issue" do
      script = write_script("/tmp/railwatch_runner_spec_probe.rb", "raise 'probe_in_tmp'")
      expect { run_runner(script) }.to raise_error("probe_in_tmp")

      expect(railwatch_records(:command).sole[:interactive]).to be(true)
      expect(railwatch_records(:exception)).to be_empty
    end

    it "treats a scratch directory as deployed once it is off interactive_runner_paths" do
      Railwatch.config.interactive_runner_paths = [ "/var/tmp/" ]
      script = write_script("/tmp/railwatch_runner_spec_probe2.rb", "raise 'probe_in_tmp'")
      expect { run_runner(script) }.to raise_error("probe_in_tmp")

      expect(railwatch_records(:command).sole[:interactive]).to be_nil
      expect(railwatch_records(:exception).sole[:message]).to eq("probe_in_tmp")
    ensure
  ...
  describe "a deployed script" do
    it "reports the exception, with no interactive flag on the command record" do
      script = write_script(Rails.root.join("script/railwatch_runner_spec_nightly.rb").to_s, "raise 'nightly_is_broken'")
      expect { run_runner(script) }.to raise_error("nightly_is_broken")

      cmd = railwatch_records(:command).sole
      expect(cmd[:interactive]).to be_nil
      expect(cmd[:exit_code]).to eq(1)
      expect(railwatch_records(:exception).sole[:message]).to eq("nightly_is_broken")
    end
  end
  ...
  describe "when the patch lands mid-perform (a real shell invocation)" do
    ...
    it "still opens the execution, and withholds the exception of a script under /tmp" do
      path = write_script("/tmp/railwatch_probe_#{Process.pid}.rb", "")
    ...
    it "still reports the exception of a deployed script" do
      ...
      expect(railwatch_records(:exception).sole).to include(message: "nightly_died")
    end
