...
RSpec.describe "Railwatch::Patches::Inertia" do
  it "is already prepended onto InertiaRails::Renderer at boot, since inertia_rails is loaded in this dummy app" do
    expect(InertiaRails::Renderer.ancestors).to include(Railwatch::Patches::Inertia::Renderer)
  end

  it "install! is a no-op (does not raise or double-prepend) when called again" do
    expect { Railwatch::Patches::Inertia.install! }.not_to raise_error
    expect(InertiaRails::Renderer.ancestors.count(Railwatch::Patches::Inertia::Renderer)).to eq(1)
  end
  ...
  describe "the prepended Renderer behavior" do
    ...
    let(:host) do
      ...
      end.tap { |klass| klass.prepend(Railwatch::Patches::Inertia::Renderer) }
    end
    ...
    it "stamps the request env with the component name on #render, then calls super" do
      ...
      expect(env["railwatch.inertia_component"]).to eq("widgets/index")
    end
    ...
    it "stamps the request env with elapsed milliseconds on #ssr_render, then calls super" do
      ...
      expect(env["railwatch.inertia_ssr_ms"]).to be_a(Float).and be >= 0
    end
