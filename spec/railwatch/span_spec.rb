...
RSpec.describe "Railwatch.span" do
  ...
  def in_execution(rate: 1.0)
    Railwatch.config.sample[:commands] = rate
  ...
  it "returns the block's value" do
    expect(in_execution { Railwatch.span("pdf.render") { 6 * 7 } }).to eq(42)
  end

  it "records one span with status ok and a duration in microseconds" do
    in_execution { Railwatch.span("pdf.render") { sleep 0.002 } }

    span = railwatch_records(:span).sole
  ...
  it "records the span as failed and re-raises the block's exception" do
    expect { in_execution { Railwatch.span("pdf.render") { raise ArgumentError, "nope" } } }
      .to raise_error(ArgumentError, "nope")

    expect(railwatch_records(:span).sole).to include(name: "pdf.render", status: "failed")
  end

  it "counts every span on the execution's spans counter" do
    in_execution { 3.times { |i| Railwatch.span("step-#{i}") { i } } }

    expect(railwatch_records(:command).sole[:counters][:spans]).to eq(3)
  end

  it "still yields, records nothing, and counts nothing when Railwatch is disabled" do
  ...
  it "still yields and records nothing outside any execution" do
    expect(Railwatch.span("pdf.render") { 42 }).to eq(42)

    expect(railwatch_records(:span)).to be_empty
  end

  it "still yields and records nothing when the execution is sampled out" do
    expect(in_execution(rate: 0.0) { Railwatch.span("pdf.render") { 42 } }).to eq(42)

    expect(railwatch_records(:span)).to be_empty
  end

  it "still yields and records nothing inside a Railwatch.ignore block" do
    result = in_execution { Railwatch.ignore { Railwatch.span("pdf.render") { 42 } } }
