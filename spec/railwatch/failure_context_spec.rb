...
RSpec.describe "failure context", type: :request do
  ...
  after do
    Railwatch.config.failure_context = 0
    Railwatch.config.tail_sample_slow_ms = nil
  end
  ...
  def log(message)
    Railwatch.record(:log, level: "info", message: message, tags: [], context: "{}")
  end

  describe "a head-sampled-out request" do
    before do
      Railwatch.config.sample[:requests] = 0.0
      Railwatch.config.failure_context = 200
    end
    ...
    it "ships the child records that led up to an unhandled exception, with the exception and the parent" do
      ...
      expect(railwatch_records(:request).sole).to include(status_code: 500, tail_sampled: true)
    end
    ...
    it "still lets a late Railwatch.sample(1.0) ship the whole execution" do
  ...
  describe "a head-sampled-out Active Job attempt" do
    before do
      Railwatch.config.sample[:jobs] = 0.0
      Railwatch.config.failure_context = 200
    end
    ...
    it "ships its queries and logs when the job raises" do
      ...
      expect(railwatch_records(:job_attempt).sole).to include(status: "failed", tail_sampled: true)
    end
  ...
  describe "a head-sampled-out scheduled task" do
    ...
    before do
      Railwatch.config.sample[:scheduled_tasks] = 0.0
      Railwatch.config.failure_context = 200
    end
    ...
    it "ships its child records when the task raises" do
      ...
      expect(railwatch_records(:query)).not_to be_empty
      expect(railwatch_records(:scheduled_task).sole).to include(task_key: "widget_failure_context", tail_sampled: true)
    end
  ...
  describe "a head-sampled-out command" do
    before { Railwatch.config.failure_context = 200 }

    it "ships the queries a failing rake task ran before it raised" do
      Rake::Task.define_task(:railwatch_failure_context_boom) do
        Widget.count
        raise ArgumentError, "kaboom"
      end
      Railwatch.config.sample[:commands] = 0.0

      expect { Rake::Task[:railwatch_failure_context_boom].execute }.to raise_error(ArgumentError)
    ...
    it "ships nothing for a rake task that completes" do
      Rake::Task.define_task(:railwatch_failure_context_ok) { Widget.count }
      Railwatch.config.sample[:commands] = 0.0

      Rake::Task[:railwatch_failure_context_ok].execute
    ...
    it "never promotes the ring for an ignored exception class" do
      start_command
      log("before")
      Railwatch.report(ActiveRecord::RecordNotFound.new("Couldn't find Widget"), handled: false)
      finish_command

      expect(railwatch_records).to be_empty
    end

    it "never promotes the ring for an exception reported inside Railwatch.ignore, shipping only the parent" do
      start_command
      log("before")
      Railwatch.ignore { Railwatch.report(RuntimeError.new("paused"), handled: false) }
      finish_command

      expect(railwatch_records(:log)).to be_empty
    ...
    it "never promotes the ring for an interactive `rails runner`, whose exceptions are the operator's" do
      ...
      log("before")
      Railwatch.report(RuntimeError.new("typo"), handled: false)
      finish_command
  ...
  describe "bounded memory" do
    ...
    it "accounts the overflow onto the batch's dropped count when the ring is promoted" do
      Railwatch.config.failure_context = 10
      start_command
      25.times { |i| log("line #{i}") }
      Railwatch.report(RuntimeError.new("boom"), handled: false)
      finish_command

      expect(Railwatch.reporter.buffer.dropped).to eq(15)
      expect(railwatch_records(:log).map { |r| r[:message] }).to eq((15..24).map { |i| "line #{i}" })
    end

    it "keeps every concurrent execution's ring bounded and separate" do
      Railwatch.config.failure_context = 50
      Railwatch.config.sample[:jobs] = 0.0

      executions = 4.times.map do |t|
        Thread.new do
          exe = Railwatch.start_execution(source: :job, sample_kind: :jobs)
  ...
  describe "Execution" do
    def sampled_out_execution
      Railwatch::Execution.new(source: :request, sampled: false)
    end
    ...
    it "records nothing for a sampled-out execution while failure_context is 0, which is the default" do
      ...
      expect(Railwatch.config.failure_context).to eq(0)
    ...
    it "leaves tail sampling's larger buffer in charge when both are configured" do
      Railwatch.config.failure_context = 2
    ...
    it "stops ringing once Railwatch.keep! takes over the execution" do
      Railwatch.config.failure_context = 2
      exe = sampled_out_execution
    ...
    it "stays paused inside Railwatch.ignore even while ringing" do
      Railwatch.config.failure_context = 200
      exe = sampled_out_execution
      exe.paused_depth += 1
