...
RSpec.describe "profile record", type: :request do
  before do
    ...
    Lantern.config.profile_interval_us = 200
  ...
  it "ships one v1 profile for a request when every request is profiled" do
    ...
    profile = lantern_records(:profile).sole
  ...
  it "carries the request's envelope, so the profile joins its execution" do
    ...
    profile = lantern_records(:profile).sole
    expect(profile[:execution_id]).to eq(lantern_records(:request).sole[:execution_id])
  ...
  it "ships the collapsed stacks gzipped and base64-encoded, with the uncompressed size" do
    ...
    expect(stacks.bytesize).to be <= Lantern::Profiler::MAX_COLLAPSED_BYTES
  ...
  it "names the dummy app's own controller frame in the collapsed stacks" do
    ...
    expect(stacks_of(lantern_records(:profile).sole))
  ...
  it "profiles with stackprof when config.profiler pins it" do
    Lantern.config.profile_sample = 1.0
    Lantern.config.profiler = :stackprof
    get "/widgets"
  ...
  it "profiles only the outer execution when one execution nests inside another" do
    ...
    inner = Lantern.start_execution(source: :job, sample_kind: :jobs)

    expect(outer.profiler_handle).not_to be_nil
    expect(inner.profiler_handle).to be_nil
    expect(Lantern::Profiler.skipped).to eq(1)

    Lantern.finish_execution(:job_attempt, group: "g", name: "InnerJob", queue: "default")
    Lantern.finish_execution(:command, group: "g", class: "Rake::Task", name: "outer",
                             command: "rake outer", exit_code: 0)

    expect(lantern_records(:profile).sole[:execution_id]).to eq(outer.id)
  end
end
