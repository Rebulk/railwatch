...
RSpec.describe Lantern::Profiler do
  after do
  ...
  def churn(milliseconds)
    deadline = Lantern::Clock.monotonic + (milliseconds / 1000.0)
    n = 0
    n += 1 while Lantern::Clock.monotonic < deadline
    n
  end
  ...
  describe "backend selection" do
    ...
    it "has no backend when config.profiler names one that isn't installed" do
      Lantern.config.profiler = :not_a_profiler
  ...
  describe "start/stop" do
    ...
    it "degrades to nil when the backend raises on stop" do
      ...
      # The stub, not Lantern, is what left vernier collecting: put the real
  ...
  describe ".collapse" do
    ...
    it "formats each frame as `Class#method (path:line)`" do
      result = stackprof_result([ [ [ "Widget#save" ], 3 ] ], files: { "Widget#save" => "#{Lantern::Backtrace.app_root}app/models/widget.rb" })
    ...
    it "caps the text at 4 MiB, dropping the least frequent stacks" do
      ...
      expect(collapsed.bytesize).to be <= Lantern::Profiler::MAX_COLLAPSED_BYTES
