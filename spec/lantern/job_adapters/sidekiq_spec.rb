...
RSpec.describe Lantern::JobAdapters::Sidekiq do
  ...
  it "marks retry-disabled and exhausted attempts failed" do
    ...
    expired = direct_payload.merge(
      "jid" => "expired", "retry_for" => 60,
      "failed_at" => Lantern::Clock.now - 61, "retry_count" => 0)
    ...
    expect { server.call(Object.new, expired, "critical") { raise "expired" } }.to raise_error("expired")

    expect(lantern_records(:job_attempt).map { |attempt| [ attempt[:job_id], attempt[:status], attempt[:attempt] ] })
      .to contain_exactly(
        [ "discarded", "failed", 1 ], [ "dead", "failed", 3 ], [ "expired", "failed", 2 ])
  end
