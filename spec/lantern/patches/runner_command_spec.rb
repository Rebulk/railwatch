...
RSpec.describe Lantern::Patches::RunnerCommand do
  ...
  describe "a script in a scratch directory" do
    it "ships an interactive command record and opens no issue" do
      script = write_script("/tmp/lantern_runner_spec_probe.rb", "raise 'probe_in_tmp'")
      expect { run_runner(script) }.to raise_error("probe_in_tmp")

      expect(lantern_records(:command).sole[:interactive]).to be(true)
      expect(lantern_records(:exception)).to be_empty
    end

    it "treats a scratch directory as deployed once it is off interactive_runner_paths" do
      Lantern.config.interactive_runner_paths = [ "/var/tmp/" ]
      script = write_script("/tmp/lantern_runner_spec_probe2.rb", "raise 'probe_in_tmp'")
      expect { run_runner(script) }.to raise_error("probe_in_tmp")

      expect(lantern_records(:command).sole[:interactive]).to be_nil
      expect(lantern_records(:exception).sole[:message]).to eq("probe_in_tmp")
    ensure
      Lantern.config.interactive_runner_paths = Lantern::Configuration::DEFAULT_INTERACTIVE_RUNNER_PATHS.dup
    end
  end
  ...
  describe "a deployed script" do
    it "reports the exception, with no interactive flag on the command record" do
      script = write_script(Rails.root.join("script/lantern_runner_spec_nightly.rb").to_s, "raise 'nightly_is_broken'")
      expect { run_runner(script) }.to raise_error("nightly_is_broken")

      cmd = lantern_records(:command).sole
      expect(cmd[:interactive]).to be_nil
      expect(cmd[:exit_code]).to eq(1)
      expect(lantern_records(:exception).sole[:message]).to eq("nightly_is_broken")
    end
  end
