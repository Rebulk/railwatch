...
RSpec.describe "Lantern::Patches::NetHttp" do
  def finish!
  ...
  it "omits the default port 443 from the url for an https request" do
    ...
    expect(lantern_records(:outgoing_request).sole[:url]).to eq("https://secure.test/x")
  end
  ...
  it "includes a non-default port in the url for an http request" do
    ...
    expect(lantern_records(:outgoing_request).sole[:url]).to eq("http://example.test:8080/x")
  end
  ...
  describe "#lantern_self_request?" do
    it "is true only when both host and port match config.ingest_url" do
      ...
      expect(same_host_and_port.send(:lantern_self_request?)).to be(true)
      expect(same_host_different_port.send(:lantern_self_request?)).to be(false)
    end
  end

  describe "REENTRY guard" do
    it "leaves Thread.current[:lantern_net_http] cleared after a request completes, so a later independent request is still instrumented" do
      Lantern.start_execution(source: :command, sample_kind: :commands)
      Net::HTTP.get(URI("http://example.test/widgets"))
      expect(Thread.current[:lantern_net_http]).to be_nil
      Net::HTTP.get(URI("http://example.test/widgets"))
      finish!

      expect(lantern_records(:outgoing_request).size).to eq(2)
    end

    it "skips instrumentation entirely for a request made while the guard is already set" do
      Lantern.start_execution(source: :command, sample_kind: :commands)
      Thread.current[:lantern_net_http] = true
      begin
        Net::HTTP.get(URI("http://example.test/widgets"))
      ensure
        Thread.current[:lantern_net_http] = nil
      end
      finish!
